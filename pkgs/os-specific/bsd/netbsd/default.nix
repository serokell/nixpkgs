...
let
  ...
  netbsd = with nbSplicedPackages; {
    ...
    mkDerivation = lib.makeOverridable (attrs:
      let stdenv' = if attrs.noCC or false then stdenvNoCC else stdenv;
      in stdenv'.mkDerivation ({
        name =
          "${attrs.pname or (baseNameOf attrs.path)}-netbsd-${attrs.version}";
        src = attrs.src or fetchNetBSD attrs.path attrs.version attrs.sha256;

        extraPaths = [ ];

        nativeBuildInputs =
          [ makeMinimal install tsort lorder mandoc groff stat ];
        buildInputs = [ compat ];
    ...
    compat = if stdenv.hostPlatform.isNetBSD then
      stdenv
    else
      mkDerivation rec {
        path = "tools/compat";
        ...
        makeFlags =
          [ "INSTALL=${buildPackages.coreutils}/bin/install" "TOOLDIR=$(out)" ];
        RENAME = "-D";
    ...
    libedit = mkDerivation {
      ...
      '';
      NIX_CFLAGS_COMPILE =
        [ "-D__noinline=" "-D__scanflike(a,b)=" "-D__va_list=va_list" ];
    };
