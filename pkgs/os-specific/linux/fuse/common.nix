...
{ stdenv, fetchFromGitHub, fetchpatch, fusePackages, utillinux, gettext, meson, ninja, pkgconfig, autoreconfHook, python3Packages, which
}:
...
in stdenv.mkDerivation rec {
  ...
  patches = stdenv.lib.optional (!isFuse3 && stdenv.isAarch64) (fetchpatch {
    url =
  ...
  nativeBuildInputs =
    if isFuse3 then [ meson ninja pkgconfig ] else [ autoreconfHook gettext ];
  ...
  meta = with stdenv.lib; {
    ...
    description =
      "Kernel module and library that allows filesystems to be implemented in user space";
