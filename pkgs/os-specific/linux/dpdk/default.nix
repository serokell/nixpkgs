...
in stdenv.mkDerivation rec {
  ...
  RTE_KERNELDIR =
    if mod then "${kernel.dev}/lib/modules/${kver}/build" else "/var/empty";
  ...
  installTargets =
    [ "install-runtime" "install-sdk" "install-kmod" ]; # skip install-doc

  installFlags = [ "prefix=$(out)" ]
    ++ lib.optionals mod [ "kerneldir=$(kmod)/lib/modules/${kver}" ];
