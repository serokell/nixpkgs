{ stdenv, fetchurl, fetchpatch, lib, pkgconfig, utillinux, libcap, libtirpc, libevent, sqlite, kerberos, kmod, libuuid, keyutils, lvm2, systemd, coreutils, tcp_wrappers, python3
}:

let statdPath = lib.makeBinPath [ systemd utillinux coreutils ];
...
in stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      "https://kernel.org/pub/linux/utils/nfs-utils/${version}/${name}.tar.xz";
  ...
  configureFlags = [
    ...
    "--with-rpcgen=${stdenv.glibc.bin}/bin/rpcgen";
  ...
  patches = [
    ...
    (fetchpatch {
      ...
      url =
        "http://git.linux-nfs.org/?p=steved/nfs-utils.git;a=commitdiff_plain;h=a8133e1fd174267536cd459e19cfe0a1cbbe037c;hp=a709f25c1da4a2fb44a1f3fd060298fbbd88aa3c";
