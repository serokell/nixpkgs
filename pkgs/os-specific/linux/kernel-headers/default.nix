...
let
  makeLinuxHeaders = { src, version, patches ? [ ] }:
    stdenvNoCC.mkDerivation {
      ...
      ARCH =
        stdenvNoCC.hostPlatform.platform.kernelArch or stdenvNoCC.hostPlatform.kernelArch;
      ...
      extraIncludeDirs =
        lib.optional stdenvNoCC.hostPlatform.isPowerPC [ "ppc" ];

      inherit patches;

      hardeningDisable =
        lib.optional stdenvNoCC.buildPlatform.isDarwin "format";
...
in {
  ...
  linuxHeaders = let version = "4.19.16";
    in makeLinuxHeaders {
      inherit version;
      src = fetchurl {
