...
let
  ...
  apparmor-sources = fetchurl {
    url =
      "https://launchpad.net/apparmor/${apparmor-series}/${apparmor-version}/+download/apparmor-${apparmor-version}.tar.gz";
  ...
  patches = stdenv.lib.optionals stdenv.hostPlatform.isMusl [
    (fetchpatch {
      url =
        "https://git.alpinelinux.org/cgit/aports/plain/testing/apparmor/0003-Added-missing-typedef-definitions-on-parser.patch?id=74b8427cc21f04e32030d047ae92caa618105b53";
      name = "0003-Added-missing-typedef-definitions-on-parser.patch";
      sha256 = "0yyaqz8jlmn1bm37arggprqz0njb4lhjni2d9c8qfqj0kll0bam0";
    })
    (fetchpatch {
      url =
        "https://git.alpinelinux.org/cgit/aports/plain/testing/apparmor/0007-Do-not-build-install-vim-file-with-utils-package.patch?id=74b8427cc21f04e32030d047ae92caa618105b53";
  ...
  libapparmor = stdenv.mkDerivation {
    ...
    nativeBuildInputs =
      [ autoreconfHook bison flex pkgconfig swig ncurses which perl ];

    buildInputs = [ ] ++ stdenv.lib.optional withPerl perl
  ...
  apparmor-utils = stdenv.mkDerivation {
    ...
    buildInputs = [ perl python libapparmor libapparmor.python ];
    ...
    makeFlags = "LANGS=";
    installFlags =
      "DESTDIR=$(out) BINDIR=$(out)/bin VIM_INSTALL_PATH=$(out)/share PYPREFIX=";
  ...
  apparmor-bin-utils = stdenv.mkDerivation {
    ...
    nativeBuildInputs = [ pkgconfig libapparmor gawk which ];
  ...
  apparmor-parser = stdenv.mkDerivation {
    ...
    postPatch = "cd ./parser";
    makeFlags =
      "LANGS= USE_SYSTEM=1 INCLUDEDIR=${libapparmor}/include AR=${stdenv.cc.bintools.targetPrefix}ar";
