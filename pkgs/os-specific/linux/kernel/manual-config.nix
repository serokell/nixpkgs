...
config ?
  stdenv.lib.optionalAttrs allowImportFromDerivation (readConfig configfile),
...
let
  inherit (stdenv.lib)
    hasAttr getAttr optional optionals optionalString optionalAttrs maintainers
    platforms;
  ...
  drvAttrs = config_: platform: kernelPatches: configfile:
    ...
    in (optionalAttrs isModular { outputs = [ "out" "dev" ]; }) // {
      ...
      patches = map (p: p.patch) kernelPatches
        ...
        ++ optional (stdenv.lib.versionAtLeast version "4.13")
      ...
      installFlags = [ "INSTALLKERNEL=${installkernel}" "INSTALL_PATH=$(out)" ]
        ++ (optional isModular "INSTALL_MOD_PATH=$(out)")
        ++ optional installsFirmware "INSTALL_FW_PATH=$(out)/lib/firmware";

      # Some image types need special install targets (e.g. uImage is installed with make uinstall)
      installTargets = [
        (if platform ? kernelInstallTarget then
          platform.kernelInstallTarget
        else if platform.kernelTarget == "uImage" then
          "uinstall"
        else if platform.kernelTarget == "zImage" || platform.kernelTarget
        == "Image.gz" then
          "zinstall"
        else
          "install")
...
in assert stdenv.lib.versionAtLeast version "4.14" -> libelf != null;
assert stdenv.lib.versionAtLeast version "4.15" -> utillinux != null;
stdenv.mkDerivation
((drvAttrs config stdenv.hostPlatform.platform kernelPatches configfile) // {
  name = "linux-${version}";
