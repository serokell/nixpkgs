...
let
  ...
  xorgVer =
    lib.concatStringsSep "." (lib.take 2 (lib.splitString "." xorgFullVer));
...
in stdenv.mkDerivation rec {
  ...
  kernelVersion =
    if libsOnly then "" else (builtins.parseDrvName kernel.name).version;
  kernelDir =
    if libsOnly then "" else "${kernel.dev}/lib/modules/${kernelVersion}";
  scriptPath = lib.concatStringsSep ":"
    (lib.optionals (!libsOnly) [ "${utillinux}/bin" "${gawk}/bin" ]);
