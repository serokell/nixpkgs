...
let
  ...
  fromBase64 = str:
    let
      ...
      padding =
        if hasSuffix "==" str then 2 else if hasSuffix "=" str then 1 else 0;
    ...
    in take quads (concatMap (c: table.${c}) chars);

  # Convert a list of quads with padding into a base16-encoded string.
  toBase16 = quads:
    if length quads == 0 then
      ""
    else if length quads == 1 then
      throw "toBase16: odd quads"
    else
      let
        hexad = 4 * elemAt quads 0 + elemAt quads 1;
        hexits = "0123456789abcdef";
      in substring hexad 1 hexits + toBase16 (drop 2 quads);
...
in let
  ...
  integrityHash = integrity:
    ...
    else if hasPrefix "sha512-" integrity then
