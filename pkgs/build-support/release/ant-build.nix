...
let
  antFlags = "-f ${buildfile} "
    + stdenv.lib.concatMapStrings ({ name, value }: "-D${name}=${value} ")
    antProperties;
  lib = stdenv.lib;
...

{
  ...
  antSetupPhase = with stdenv.lib; ''
    ...
    for abi in ${
      concatStringsSep " " (map (f: "`find ${f} -name '*.jar'`") antBuildInputs)
    }; do
...

{
  ...
  buildInputs = [ ant jre zip unzip ]
    ++ stdenv.lib.optional (args ? buildInputs) args.buildInputs;
