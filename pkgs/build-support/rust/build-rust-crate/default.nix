...
let
  ...
  target_os = if stdenv.hostPlatform.isDarwin then
    "macos"
  else
  ...
  makeDeps = dependencies:
    (lib.concatMapStringsSep " " (dep:
      let extern = lib.strings.replaceStrings [ "-" ] [ "_" ] dep.libName;
...
lib.makeOverridable ({ rust, release, verbose, features, buildInputs
  ...
  let
    ...
    extraDerivationAttrs =
      lib.filterAttrs (n: v: !lib.elem n processedAttrs) crate;
  ...
  in stdenv.mkDerivation (rec {
    ...
    buildDependencies = builtins.map (dep:
      dep.override {
        rust = rust;
        release = release;
        verbose = verbose;
    ...
    completeDeps = lib.lists.unique (dependencies
      ++ lib.lists.concatMap (dep: dep.completeDeps) dependencies);
    completeBuildDeps = lib.lists.unique (buildDependencies
      ++ lib.lists.concatMap (dep: dep.completeBuildDeps ++ dep.completeDeps)
      buildDependencies);
    ...
    depsMetadata = builtins.foldl' (str: dep: str + dep.metadata) ""
      (dependencies ++ buildDependencies);
    metadata = builtins.substring 0 10 (builtins.hashString "sha256" (crateName
      + "-" + crateVersion + "___" + toString crateFeatures + "___"
      + depsMetadata));
    ...
    crateBin = if crate ? crateBin then
      builtins.foldl' (bins: bin:
        ...
        in bins + (if bin == "" then "" else ",") + "${name} ${path}"
    ...
    extraLinkFlags =
      builtins.concatStringsSep " " (crate.extraLinkFlags or [ ]);
    edition = crate.edition or null;
    extraRustcOpts =
      (if crate ? extraRustcOpts then crate.extraRustcOpts else [ ])
      ++ extraRustcOpts_
      ++ (lib.optional (edition != null) "--edition ${edition}");

    configurePhase = configureCrate {
      inherit crateName buildDependencies completeDeps completeBuildDeps
        crateDescription crateFeatures libName build workspace_member release
        libPath crateVersion extraLinkFlags extraRustcOpts crateAuthors
        crateHomepage verbose colors target_os;
    };
    buildPhase = buildCrate {
      inherit crateName dependencies crateFeatures libName release libPath
        crateType metadata crateBin hasCrateBin verbose colors extraRustcOpts;
    };
