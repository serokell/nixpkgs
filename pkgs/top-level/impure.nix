...
in { # We combine legacy `system` and `platform` into `localSystem`, if
...
system ? localSystem.system, platform ? localSystem.platform, crossSystem ? null
...
import ./. (builtins.removeAttrs args [ "system" "platform" ] // {
  ...
  localSystem = if builtins.isString localSystem then
    localSystem
  else
    (if args ? localSystem then { } else { system = builtins.currentSystem; })
    // localSystem;
})
