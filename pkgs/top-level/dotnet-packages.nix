{ stdenv, pkgs, buildDotnetPackage, fetchurl, fetchFromGitHub, fetchNuGet, pkgconfig, mono, fsharp, unzip, overrides ?
  { } }:
...
let
  ...
  dotnetPackages = with self; {
    ...
    ExcelDna = buildDotnetPackage rec {
      ...
      outputFiles = [
        "Source/ExcelDnaPack/bin/Release/*"
    ...
    ExcelDnaRegistration = buildDotnetPackage rec {
      ...
      buildInputs = [ fsharp dotnetPackages.ExcelDna ];

      xBuildFiles = [
        "Source/ExcelDna.Registration/ExcelDna.Registration.csproj"
        "Source/ExcelDna.Registration.FSharp/ExcelDna.Registration.FSharp.fsproj"
      ];
      outputFiles = [
        "Source/ExcelDna.Registration/bin/Release/*"
        "Source/ExcelDna.Registration.FSharp/bin/Release/*FSharp*"
      ];
    ...
    ExtCore = buildDotnetPackage rec {
      ...
      buildInputs = [ fsharp dotnetPackages.NUnit dotnetPackages.FsCheck ];
    ...
    FSharpCompilerService = buildDotnetPackage rec {
      ...
      buildInputs = [ fsharp dotnetPackages.NUnit ];

      outputFiles = [ "bin/v4.5/*" ];

      meta = {
        description =
          "The F# compiler services package is a component derived from the F# compiler source code that exposes additional functionality for implementing F# language bindings";
    ...
    FSharpData = buildDotnetPackage rec {
      ...
      fileProvidedTypes = fetchurl {
        ...
        url =
          "https://raw.githubusercontent.com/fsprojects/FSharp.TypeProviders.StarterPack/877014bfa6244ac382642e113d7cd6c9bc27bc6d/src/ProvidedTypes.fs";
      ...
      fileDebugProvidedTypes = fetchurl {
        ...
        url =
          "https://raw.githubusercontent.com/fsprojects/FSharp.TypeProviders.StarterPack/877014bfa6244ac382642e113d7cd6c9bc27bc6d/src/DebugProvidedTypes.fs";
      ...
      xBuildFiles =
        [ "src/FSharp.Data.fsproj" "src/FSharp.Data.DesignTime.fsproj" ];
    ...
    MathNetNumerics = buildDotnetPackage rec {
      ...
      src = fetchurl {
        name = "${baseName}-${version}.tar.gz";
        url =
          "https://github.com/mathnet/mathnet-numerics/archive/v${version}.tar.gz";
    ...
    Projekt = buildDotnetPackage rec {
      ...
      buildInputs =
        [ fsharp dotnetPackages.UnionArgParser dotnetPackages.FsUnit ];
    ...
    UnionArgParser = buildDotnetPackage rec {
      ...
      buildInputs = [ fsharp dotnetPackages.NUnit dotnetPackages.FsUnit ];
