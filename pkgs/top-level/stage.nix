...
let
  ...
  platformCompat = self: super:
    let inherit (super.stdenv) buildPlatform hostPlatform targetPlatform;
    in {
  ...
  aliases = self: super:
    ...
    (import ./aliases.nix lib self super);
  ...
  otherPackageSets = self: super: {
    ...
    # supported.
    pkgsMusl = if stdenv.hostPlatform.isLinux then
      nixpkgsFun {
      ...
      throw "Musl libc only supports Linux systems.";
    ...
    appendOverlays = extraOverlays:
      if extraOverlays == [ ] then
        ...
        import ./stage.nix
        (args // { overlays = args.overlays ++ extraOverlays; });
