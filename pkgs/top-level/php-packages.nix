...
let
  self = with self; {
    ...
    mkDerivation = { pname, ... }@args:
      pkgs.stdenv.mkDerivation (args // { pname = "php-${pname}"; });
    ...
    oci8 = buildPecl rec {
      ...
      configureFlags =
        [ "--with-oci8=shared,instantclient,${pkgs.oracle-instantclient}/lib" ];
    };
    ...
    php_excel = buildPecl rec {
      ...
      src = pkgs.fetchurl {
        url =
          "https://github.com/iliaal/php_excel/releases/download/Excel-1.0.2-PHP7/excel-${version}-${phpVersion}.tgz";
    ...
    pthreads = assert (pkgs.config.php.zts or false);
      buildPecl rec {
        version = "3.1.5";
        pname = "pthreads";
