{ stdenv, fetchFromGitHub, bzip2, nix, perl, perlPackages, }:
...
in stdenv.mkDerivation rec {
  ...
  installPhase = ''
    ...
    #! ${stdenv.shell}
    PATH=${
      makeBinPath [ bzip2 nix ]
    }:\$PATH PERL5LIB=$PERL5LIB exec ${perlPackages.Starman}/bin/starman $out/libexec/nix-serve/nix-serve.psgi "\$@"
    EOF
