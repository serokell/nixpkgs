...
stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      "https://github.com/${pname}/${pname}/releases/download/${version}/${pname}-${version}.tar.gz";
  ...
  nativeBuildInputs =
    [ pkgconfig autoconf automake gnum4 libtool perl gettext ];
  buildInputs = [
    readline
    uthash
    woff2
  ...
  configureFlags = [ "--enable-woff2" ] ++ lib.optionals (!withPython) [
