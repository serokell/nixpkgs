{ fetchzip, libX11, libGLU_combined, makeWrapper, stdenv }:

let

  libPath = stdenv.lib.makeLibraryPath [ libGLU_combined stdenv.cc.cc libX11 ];
...
in stdenv.mkDerivation rec {
  ...
  src = fetchzip {
    url =
      "http://www.kisslicer.com/uploads/1/5/3/8/15381852/kisslicer_linux64_1.6.3_release.zip";
  ...
  buildInputs = [ makeWrapper libGLU_combined libX11 ];
