{ stdenv, fetchurl, pkgconfig, curl, libevent, libiconv, libxml2, openssl, pcre
, zlib, jabberSupport ? true, iksemel, ldapSupport ? true, openldap
, odbcSupport ? true, unixODBC, snmpSupport ? true, net_snmp, sshSupport ? true
, libssh2, mysqlSupport ? false, mysql, postgresqlSupport ? false, postgresql }:
...
stdenv.mkDerivation {
  ...
  src = fetchurl {
    url =
      "mirror://sourceforge/zabbix/ZABBIX%20Latest%20Stable/${version}/zabbix-${version}.tar.gz";
    inherit sha256;
  };
  ...
  buildInputs = [ curl libevent libiconv libxml2 openssl pcre zlib ]
    ...
    ++ optional ldapSupport openldap ++ optional snmpSupport net_snmp
    ++ optional sshSupport libssh2 ++ optional mysqlSupport mysql.connector-c
