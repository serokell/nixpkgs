...
let # in mariadb # spans the whole file
  ...
  mariadb = everything // {
    inherit
      client; # libmysqlclient.so in .out, necessary headers in .dev and utils in .bin
    server =
      everything; # a full single-output build, including everything in `client` again
  ...
  common = rec { # attributes common to both builds
    ...
    buildInputs = [ ncurses openssl zlib pcre jemalloc libiconv ]
    ...
    patches = [ ./cmake-includedir.patch ] ++ optionals stdenv.isDarwin [
  ...
  client = stdenv.mkDerivation (common // {
    ...
    cmakeFlags = common.cmakeFlags
      ++ [ "-DWITHOUT_SERVER=ON" "-DWITH_WSREP=OFF" ];
  ...
  everything = stdenv.mkDerivation (common // {
    ...
    buildInputs = common.buildInputs
      ++ [ xz lzo lz4 bzip2 snappy libxml2 boost judy libevent cracklib ]
  ...
  connector-c = stdenv.mkDerivation rec {
    ...
    src = fetchurl {
      url =
        "https://downloads.mariadb.org/interstitial/connector-c-${version}/mariadb-connector-c-${version}-src.tar.gz/from/http%3A//nyc2.mirrors.digitalocean.com/mariadb/";
    ...
    cmakeFlags =
      [ "-DWITH_EXTERNAL_ZLIB=ON" "-DMYSQL_UNIX_ADDR=/run/mysqld/mysqld.sock" ];
