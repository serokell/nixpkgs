...
in self: super: {
  ...
  libXfont = super.libXfont.overrideAttrs (attrs: {
    ...
    configureFlags = lib.optionals isDarwin [ "CFLAGS=-O0" ];
  });
  ...
  xf86inputmouse = super.xf86inputmouse.overrideAttrs
    (attrs: { installFlags = "sdkdir=\${out}/include/xorg"; });
  ...
  xf86inputsynaptics = super.xf86inputsynaptics.overrideAttrs (attrs: {
    ...
    installFlags =
      "sdkdir=\${out}/include/xorg configdir=\${out}/share/X11/xorg.conf.d";
  });
  ...
  xorgserver = with self;
    ...
    in if (!isDarwin) then {
      ...
      propagatedBuildInputs = [ libpciaccess epoxy ]
        ++ commonPropagatedBuildInputs ++ lib.optionals stdenv.isLinux [ udev ];
  ...
  xf86videointel = super.xf86videointel.overrideAttrs (attrs: {
    ...
    buildInputs = attrs.buildInputs
      ++ [ self.libXfixes self.libXScrnSaver self.pixman ];
    nativeBuildInputs = attrs.nativeBuildInputs
  ...
  xwd = super.xwd.overrideAttrs
    (attrs: { buildInputs = with self; attrs.buildInputs ++ [ libXt ]; });
