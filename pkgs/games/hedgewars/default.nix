...
let
  ghc = ghcWithPackages (pkgs:
    with pkgs; [
      network
      vector
      utf8-string # broken: bytestring-show
      random
      hslogger
      SHA
      entropy
...
in stdenv.mkDerivation rec {
  ...
  postInstall = ''
    wrapProgram $out/bin/hwengine --prefix LD_LIBRARY_PATH : $LD_LIBRARY_PATH:${
      stdenv.lib.makeLibraryPath [ libGLU_combined freeglut physfs ]
    }
  '';
