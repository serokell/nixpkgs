{ stdenv, config, fetchurl, patchelf, makeWrapper, gtk2, glib, udev, alsaLib, atk, nspr, fontconfig, cairo, pango, nss, freetype, gnome2, gdk_pixbuf, curl, systemd, xorg
}:
...
stdenv.mkDerivation {
  ...
  src = fetchurl { inherit (config.planetary_annihilation) url sha256; };
  ...
  installPhase = ''
    ...
    }:{stdenv.cc.cc.lib}/lib64:${stdenv.glibc.out}/lib64" "$out/host/CoherentUI_Host"
    ...
    }:${stdenv.cc.cc.lib}/lib64:${stdenv.glibc.out}/lib64"
    ...
    for f in $out/lib/*; do
      ...
      }:${stdenv.cc.cc.lib}/lib64:${stdenv.glibc.out}/lib64" $f
    done
  '';
