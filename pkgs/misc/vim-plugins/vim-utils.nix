...
let
  ...
  vimrcFile =
    ...
    let
      ...
      pathogenImpl = lib.optionalString (pathogen != null) (let
        knownPlugins = pathogen.knownPlugins or vimPlugins;

        plugins = findDependenciesRecursively
          (map (pluginToDrv knownPlugins) pathogen.pluginNames);
      ...
      /* vim-addon-manager = VAM
         ...
         * still is minimalistic (only loads one file), the "check out" code it also
           ...
           yet
      */
      vamImpl = lib.optionalString (vam != null) (let
        ...
        toNix = x:
          if (builtins.isString x) then
            "'${x}'"
          else if builtins.isAttrs x && builtins ? out then
            toNix "${x}" # a derivation
          else if builtins.isAttrs x then
            "{${
              lib.concatStringsSep ", "
              (lib.mapAttrsToList (n: v: "${toNix n}: ${toNix v}") x)
            }}"
          else if builtins.isList x then
            "[${lib.concatMapStringsSep ", " toNix x}]"
          else
            throw "turning ${
      ...
      nativeImpl = lib.optionalString (packages != null) (let
        link = (packageName: dir: pluginPath:
          "ln -sf ${pluginPath}/share/vim-plugins/* $out/pack/${packageName}/${dir}");
...
in rec {
  ...
  vimWithRC =
    ...
    let
      ...
      vimWrapperScript = writeScriptBin vimExecutableName ''
        #!${runtimeShell}
        exec ${vimExecutable} ${rcOption "u" vimrcFile} ${
          rcOption "U" gvimrcFile
        } "$@"
      '';
      gvimWrapperScript = writeScriptBin gvimExecutableName ''
        #!${stdenv.shell}
        exec ${gvimExecutable} ${rcOption "u" vimrcFile} ${
          rcOption "U" gvimrcFile
        } "$@"
      '';
    in buildEnv {
      inherit name;
      paths = [ vimWrapperScript ] ++ lib.optional wrapGui gvimWrapperScript
  ...
  pluginnames2Nix = { name, namefiles }:
    vim_configurable.customize {
      inherit name;
      vimrcConfig.vam.knownPlugins = vimPlugins;
      ...
      vimrcConfig.customRC = ''
        ...
        ${lib.concatMapStrings (file: ''
          let opts.plugin_dictionaries += map(readfile("${file}"), 'eval(v:val)')
        '') namefiles}
  ...
  inherit (import ./build-vim-plugin.nix { inherit stdenv rtpPath vim; })
  ...
  requiredPlugins =
    ...
    let
      ...
      knownPlugins = if givenKnownPlugins != null then
        givenKnownPlugins
      else if vam != null && vam ? knownPlugins then
        vam.knownPlugins
      else if pathogen != null && pathogen ? knownPlugins then
        pathogen.knownPlugins
      else
        vimPlugins;
      pathogenPlugins = findDependenciesRecursively
        (map (pluginToDrv knownPlugins) pathogen.pluginNames);
      vamPlugins = findDependenciesRecursively (map (pluginToDrv knownPlugins)
        (lib.concatMap vamDictToNames vam.pluginDictionaries));
      ...
      nativePlugins = lib.concatMap
        ({ start ? [ ], opt ? [ ], knownPlugins ? vimPlugins }: start ++ opt)
  ...
  test_nvim_with_vim_nix_using_pathogen =
    neovim.override { configure.pathogen.pluginNames = [ "vim-nix" ]; };
