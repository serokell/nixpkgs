...
let
  ...
  mktplcExtRefToFetchArgs = ext: {
    url =
      "https://${ext.publisher}.gallery.vsassets.io/_apis/public/gallery/publisher/${ext.publisher}/extension/${ext.name}/${ext.version}/assetbyname/Microsoft.VisualStudio.Services.VSIXPackage";
    sha256 = ext.sha256;
  ...
  buildVscodeExtension = a@{ name, namePrefix ? "${extendedPkgName}-extension-"
    ...
    stdenv.mkDerivation ((removeAttrs a [ "vscodeExtUniqueId" ]) // {
      ...
      installPrefix =
        "share/${extendedPkgName}/extensions/${vscodeExtUniqueId}";
  ...
  buildVscodeMarketplaceExtension = a@{ name ? "", src ? null, mktplcRef, ... }:
    assert "" == name;
    assert null == src;
  ...
  mktplcRefAttrList = [ "name" "publisher" "version" "sha256" ];

  mktplcExtRefToExtDrv = ext:
    buildVscodeMarketplaceExtension
    ((removeAttrs ext mktplcRefAttrList) // { mktplcRef = ext; });
