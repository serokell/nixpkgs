...
stdenv.mkDerivation
((lib.optionalAttrs (buildScript != null) { builder = buildScript; }) // rec {
  inherit name src configureFlags;

  nativeBuildInputs = [ pkgconfig fontforge makeWrapper flex bison ];
  ...
  # LD_LIBRARY_PATH.
  NIX_LDFLAGS = map (path: "-rpath " + path) (map (x: "${lib.getLib x}/lib")
  ...
  postInstall =
    let links = prefix: pkg: "ln -s ${pkg} $out/${prefix}/${pkg.name}";
    in ''
