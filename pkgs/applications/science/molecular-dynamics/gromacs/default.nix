{ stdenv, fetchurl, cmake, singlePrec ? true, mpiEnabled ? false, fftw, openmpi
}:
...
stdenv.mkDerivation {
  ...
  buildInputs = [ cmake fftw ] ++ (stdenv.lib.optionals mpiEnabled [ openmpi ]);

  cmakeFlags = ''
    ${if singlePrec then
      "-DGMX_DOUBLE=OFF"
    else
      "-DGMX_DOUBLE=ON -DGMX_DEFAULT_SUFFIX=OFF"}
    ${if mpiEnabled then ''
      -DGMX_MPI:BOOL=TRUE 
