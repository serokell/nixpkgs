...
let
  ...
  kodiDependency = { name, version, rev, sha256, ... }@attrs:
    let attrs' = builtins.removeAttrs attrs [ "name" "version" "rev" "sha256" ];
...
in stdenv.mkDerivation rec {
  ...
  buildInputs = [
  ...
  ] ++ lib.optional dbusSupport dbus ++ lib.optionals joystickSupport [ cwiid ]
    ...
    ++ lib.optional udevSupport udev ++ lib.optional usbSupport libusb
    ++ lib.optional vdpauSupport libvdpau ++ lib.optional useWayland [
      wayland
      waylandpp
  ...
  nativeBuildInputs = [
    ...
    autoconf
    automake
    libtool # still needed for some components. Check if that is the case with 18.0
  ...
  passthru = { pythonPackages = python2Packages; };
