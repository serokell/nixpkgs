...
let primary-src = import ./default-primary-src.nix { inherit fetchurl; };
...
in let
  ...
  srcs = {
    third_party = map (x:
      ...
      })) ((import ./libreoffice-srcs.nix) ++ [(rec {
        name = "unowinreg.dll";
...
in stdenv.mkDerivation rec {
  ...
  nativeBuildInputs =
    [ wrapGAppsHook gdb fontforge autoconf automake bison pkgconfig libtool ];
