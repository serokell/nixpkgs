...
let
  paperless = stdenv.mkDerivation rec {
    ...
    pythonCheckEnv =
      python.withPackages (_: (runtimePackages ++ checkPackages));
  ...
  python = python3.override {
    packageOverrides = self: super: {
      ...
      django-crispy-forms =
        self.callPackage ./python-modules/django-crispy-forms.nix { };
    };
  };

  django_2_0 = pyPkgs:
    pyPkgs.django_2_1.overrideDerivation (_: rec {
      pname = "Django";
      version = "2.0.12";
      name = "${pname}-${version}";
  ...
  pyocrWithUserTesseract = pyPkgs:
    let pyocr = pyPkgs.pyocr.override { inherit tesseract; };
    in if pyocr.outPath == pyPkgs.pyocr.outPath then
      pyocr
    else
    ...
    # avoid build errors.
      pyocr.overridePythonAttrs (attrs: { doCheck = false; });
in paperless
