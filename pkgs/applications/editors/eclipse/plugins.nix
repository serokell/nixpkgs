...
rec {
  ...
  buildEclipsePlugin =
    ...
    in buildEclipsePluginBase (attrs // {
  ...
  ansi-econsole = buildEclipsePlugin rec {
    ...
    meta = with stdenv.lib; {
      ...
      description =
        "Adds support for ANSI escape sequences in the Eclipse console";
  ...
  anyedittools = buildEclipsePlugin rec {
    ...
    srcFeature = fetchurl {
      url =
        "http://andrei.gmxhome.de/eclipse/features/AnyEditTools_${version}.jar";
      sha256 = "1wqzl7wq85m9gil8rnvly45ps0a2m0svw613pg6djs5i7amhnayh";
    };

    srcPlugin = fetchurl {
      url =
        "https://github.com/iloveeclipse/anyedittools/releases/download/2.7.1/de.loskutov.anyedit.AnyEditTools_${version}.jar";
  ...
  bytecode-outline = buildEclipsePlugin rec {
    ...
    srcFeature = fetchurl {
      url =
        "http://andrei.gmxhome.de/eclipse/features/de.loskutov.BytecodeOutline.feature_${version}.jar";
      sha256 = "0yciqhcq0n5i326mwy57r4ywmkz2c2jky7r4pcmznmhvks3z65ps";
    };

    srcPlugin = fetchurl {
      url =
        "http://dl.bintray.com/iloveeclipse/plugins/de.loskutov.BytecodeOutline_${version}.jar";
    ...
    meta = with stdenv.lib; {
      ...
      description =
        "Shows disassembled bytecode of current java editor or class file";
  ...
  cdt = buildEclipseUpdateSite rec {
    ...
    src = fetchzip {
      stripRoot = false;
      url =
        "https://www.eclipse.org/downloads/download.php?r=1&nf=1&file=/tools/cdt/releases/9.8/${name}/${name}.zip";
  ...
  color-theme = buildEclipsePlugin rec {
    ...
    meta = with stdenv.lib; {
      ...
      description =
        "Plugin to switch color themes conveniently and without side effects";
      license = licenses.epl10;
      platforms = platforms.all;
      maintainers = [ maintainers.rycee ];
    };
  ...
  findbugs = buildEclipsePlugin rec {
    ...
    meta = with stdenv.lib; {
      ...
      description =
        "Plugin that uses static analysis to look for bugs in Java code";
      license = licenses.epl10;
      platforms = platforms.all;
      maintainers = [ maintainers.rycee ];
    };
  ...
  gnuarmeclipse = buildEclipseUpdateSite rec {
    ...
    src = fetchzip {
      stripRoot = false;
      url =
        "https://github.com/gnuarmeclipse/plug-ins/releases/download/v${version}/ilg.gnuarmeclipse.repository-${version}.zip";
  ...
  jdt = buildEclipseUpdateSite rec {
    ...
    src = fetchzip {
      stripRoot = false;
      url =
        "https://www.eclipse.org/downloads/download.php?r=1&nf=1&file=/eclipse/downloads/drops4/R-4.12-201906051800/org.eclipse.jdt-4.12.zip";
