{ stdenv, fetchurl, autoreconfHook, libtool, pkgconfig, file, zip, wxGTK, gtk2, contribPlugins ?
  false, hunspell, gamin, boost }:

with stdenv.lib;

stdenv.mkDerivation rec {
  name =
    "${pname}-${stdenv.lib.optionalString contribPlugins "full-"}${version}";
  version = "17.12";
  pname = "codeblocks";

  src = fetchurl {
    url =
      "mirror://sourceforge/codeblocks/Sources/${version}/codeblocks_${version}.tar.xz";
  ...
  postConfigure = optionalString stdenv.isLinux
    "substituteInPlace libtool --replace ldconfig ${stdenv.cc.libc.bin}/bin/ldconfig";
  configureFlags = [ "--enable-pch=no" ] ++ optional contribPlugins [
