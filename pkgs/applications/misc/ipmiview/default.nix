...
stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      "ftp://ftp.supermicro.com/utility/IPMIView/Linux/IPMIView_${version}_build.${buildVersion}_bundleJRE_Linux_x64.tar.gz";
  ...
  buildPhase = with xorg; ''
    patchelf --set-rpath "${
      stdenv.lib.makeLibraryPath [ libX11 libXext libXrender libXtst libXi ]
    ...
    }" --set-interpreter "$(cat $NIX_CC/nix-support/dynamic-linker)" ./jre/bin/javaws
  ...
  meta = with stdenv.lib; { license = licenses.unfree; };
}
