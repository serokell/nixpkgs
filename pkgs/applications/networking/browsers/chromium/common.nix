...
let
  ...
  versionRange = min-version: upto-version:
    let
      ...
      result = versionAtLeast version min-version
        && versionOlder version upto-version;
      stable-version = (import ./upstream-info.nix).stable.version;
    in if versionAtLeast stable-version upto-version then
      warn
      "chromium: stable version ${stable-version} is newer than a patchset bounded at ${upto-version}. You can safely delete it."
      result
    else
      result;
  ...
  base = rec {
    ...
    buildInputs = defaultDependencies ++ [
      ...
      ++ optionals cupsSupport [ libgcrypt cups ] ++ optional useVaapi libva
    ...
    patches = optional enableWideVine ./patches/widevine.patch ++ [
      ...
      fetchpatch {
        url =
          "https://raw.githubusercontent.com/OSSystems/meta-browser/e4a667deaaf9a26a3a1aeb355770d1f29da549ad/recipes-browser/chromium/files/aarch64-skia-build-fix.patch";
    ...
    } // optionalAttrs useVaapi { use_vaapi = true; }
...
(base // removeAttrs extraAttrs [ "name" "gnFlags" "buildTargets" ])
