...
let
  ...
  mkPluginInfo =
    { output ? "out", allowedVars ? [ output ], flags ? [ ], envVars ? { } }:
    let
  ...
  widevine = stdenv.mkDerivation {
    ...
    unpackCmd = let
      chan = if upstream-info.channel == "dev" then
        "chrome-unstable"
      else if upstream-info.channel == "stable" then
        "chrome"
      else
  ...
  flash = stdenv.mkDerivation rec {
    ...
    src = fetchzip {
      url =
        "https://fpdownload.adobe.com/pub/flashplayer/pdc/${version}/flash_player_ppapi_linux.x86_64.tar.gz";
