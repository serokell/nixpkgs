...
let
  ...
  sources = if builtins.pathExists ./upstream-info.nix then
    import ./upstream-info.nix
  else
    { };
...
in rec {
  getChannel = channel:
    let chanAttrs = builtins.getAttr channel sources;
    in {
      inherit channel;
      ...
      binary = fetchurl (let
        mkUrls = arch:
          let mkURLForMirror = getDebURL channel chanAttrs.version arch;
        ...
        in if stdenv.is64bit && chanAttrs ? sha256bin64 then {
          ...
          throw "No Chrome plugins are available for your architecture.");
    };

  update = let
    csv2nix = name: src:
      import (runCommand "${name}.nix" { src = builtins.fetchurl src; } ''
    ...
    channels = lib.fold lib.recursiveUpdate { } (map (attrs: {
      ${attrs.os}.${attrs.channel} = attrs // {
        history = let
          ...
          cond = h:
            attrs.os == h.os && attrs.channel == h.channel
    ...
    */
    ...
    fetchLatest = channel:
      let result = tryFetch (mkVerURL channel.version);
      in if result.success then
        result.value
      else
        fetchLatest (channel // {
          version = if channel.history != [ ] then
            (lib.head channel.history).version
          else
            throw "Unfortunately there's no older version than "
    ...
    isLatest = channel: version:
      let ourVersion = sources.${channel}.version or null;
      in if ourVersion == null then
        false
      else
        lib.versionOlder version sources.${channel}.version || version
        == sources.${channel}.version;
    ...
    linuxChannels = let
      genLatest = channelName: channel:
        let
          ...
          keepOld = let oldChannel = sources.${channelName};
            in {
        ...
        in if isLatest channelName channel.version then
          keepOld
        else
          newUpstream;
    ...
    getLinuxFlash = channelName: channel:
      let
        ...
        isNew = arch: attr:
          !(builtins.hasAttr attr channel) && packages.${arch}.success;
    ...
    dumpAttrs = indent: attrs:
      let
        ...
        mkAttr = key: val: ''
          ${mkIndent (indent + 1)}${key} = ${mkVal val};
        '';
