...
let
  ...
  browserVersion = if isIceCatLike then
    icversion
  else if isTorBrowserLike then
    tbversion
  else
    ffversion;

  browserPatches = [ ./env_var_for_system_dir.patch ] ++ lib.optional
    (lib.versionAtLeast ffversion "63" && lib.versionOlder ffversion "69")
...
in stdenv.mkDerivation rec {
  ...
  buildInputs = [
    ...
    ++ lib.optional gtk3Support gtk3 ++ lib.optional gssSupport kerberos
  ...
  NIX_CFLAGS_COMPILE = [ "-I${glib.dev}/include/gio-unix-2.0" ]
    ++ lib.optionals (!isTorBrowserLike) [ "-I${nss.dev}/include/nss" ];

  postPatch = lib.optionalString
    (lib.versionAtLeast ffversion "63.0" && !isTorBrowserLike) ''
  ...
  nativeBuildInputs =
    ...
    ++ lib.optionals (lib.versionAtLeast ffversion "63.0") [
      rust-cbindgen
      nodejs
    ] ++ lib.optionals (lib.versionAtLeast ffversion "67.0")
  ...
  configureFlags = [
  ...
  ] ++ lib.optional (lib.versionOlder ffversion "64")
    "--disable-maintenance-service"
    ++ lib.optional (stdenv.isDarwin && lib.versionAtLeast ffversion "61")
    ...
    ++ flag ffmpegSupport "ffmpeg" ++ flag gssSupport "negotiateauth"
    ++ flag webrtcSupport "webrtc" ++ flag crashreporterSupport "crashreporter"
