...
let
  generic = overrides: source:
    stdenv.mkDerivation (source // overrides // {
      ...
      buildInputs = [ boost protobuf3_6 avahi ] ++ {
        qt4 = [ qt4 ];
        qt5 = [ qt5.qtbase ];
      }."qt${toString source.qtVersion}" ++ (overrides.buildInputs or [ ]);
  ...
  client = source:
    generic {
      ...
      buildInputs = [ libopus libsndfile speex ]
        ...
        ++ optional stdenv.isLinux alsaLib ++ optional jackSupport libjack2
        ++ optional speechdSupport speechd
        ++ optional pulseSupport libpulseaudio;

      configureFlags = [ "CONFIG+=no-server" ];
  ...
  server = generic {
    ...
    configureFlags = [ "CONFIG+=no-client" ];
  ...
  stableSource = rec {
    ...
    src = fetchurl {
      url =
        "https://github.com/mumble-voip/mumble/releases/download/${version}/mumble-${version}.tar.gz";
