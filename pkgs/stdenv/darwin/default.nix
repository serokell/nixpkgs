...
, bootstrapFiles ? let
  fetch = { file, sha256, executable ? true }:
    import <nix/fetchurl.nix> {
      url =
        "http://tarballs.nixos.org/stdenv-darwin/x86_64/d5bdfcbfe6346761a332918a267e82799ec954d2/${file}";
      ...
      with stage0; rec {
        ...
        llvmPackages_7 = {
          libcxx = stdenv.mkDerivation {
            ...
            linkCxxAbi = false;
            setupHook =
              ../../development/compilers/llvm/3.9/libc++/setup-hook.sh;
          };
  ...
  stage1 = prevStage:
    let
      persistent = self: super:
        with prevStage; {
          ...
          python = super.callPackage
            ../../development/interpreters/python/cpython/2.7/boot.nix {
          ...
          darwin = super.darwin // {
            cctools =
              super.darwin.cctools.override { enableTapiSupport = false; };
          };
        };
    in with prevStage;
    stageFun 1 prevStage {
      ...
      allowedRequisites = [ bootstrapTools ]
        ++ (with pkgs; [ libcxx libcxxabi ]) ++ [ pkgs.darwin.Libsystem ];
      ...
      persistent = self: super:
        with prevStage; {
          inherit zlib patchutils m4 scons flex perl bison unifdef unzip openssl
            python libxml2 gettext sharutils gmp libarchive ncurses pkg-config
            ...
            automake libtool ed cpio coreutils libssh2 nghttp2 libkrb5 python2
            ninja;
      ...
      persistent = self: super:
        with prevStage; {
          inherit patchutils m4 scons flex perl bison unifdef unzip openssl
            python gettext sharutils libarchive pkg-config groff bash subversion
      ...
      persistent = self: super:
        with prevStage; {
          inherit gnumake gzip gnused bzip2 gawk ed xz patch bash ncurses libffi
            zlib gmp pcre gnugrep coreutils findutils diffutils patchutils
            ninja;
          ...
          darwin = super.darwin // rec {
            ...
            cctools =
              super.darwin.cctools.override { enableTapiSupport = false; };
            libxml2-nopython =
              super.libxml2.override { pythonSupport = false; };
  ...
  stdenvDarwin = prevStage:
    let
      ...
      persistent = self: super:
        ...
        {
          inherit gnumake gzip gnused bzip2 gawk ed xz patch bash ncurses libffi
            zlib llvm gmp pcre gnugrep coreutils findutils diffutils patchutils;

          llvmPackages_7 = super.llvmPackages_7 // (let
            tools = super.llvmPackages_7.tools.extend
              (_: super: { inherit (llvmPackages_7) llvm clang-unwrapped; });
    ...
    in import ../generic rec {
      ...
      overrides = lib.composeExtensions persistent (self: super: {
        ...
        darwin = super.darwin // {
          xnu = super.darwin.xnu.override {
            python = super.python.override { configd = null; };
          };
        };
      });
    };
