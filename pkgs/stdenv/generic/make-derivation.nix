...
in rec {
  ...
  mkDerivation = {
    ...
    let
      ...
      doInstallCheck' = doInstallCheck && stdenv.hostPlatform
      ...
      defaultHardeningFlags = if stdenv.hostPlatform.isMusl then
        supportedHardeningFlags
      else
        ...
        derivationArg = (removeAttrs attrs [
          ...
          && (attrs ? name || (attrs ? pname && attrs ? version)))) {
            ...
            cmakeFlags = (if lib.isString cmakeFlags then
              ...
              cmakeFlags)
              ++ lib.optional (stdenv.hostPlatform.uname.system != null)
              "-DCMAKE_SYSTEM_NAME=${stdenv.hostPlatform.uname.system}"
              ++ lib.optional (stdenv.hostPlatform.uname.processor != null)
              "-DCMAKE_SYSTEM_PROCESSOR=${stdenv.hostPlatform.uname.processor}"
              ++ lib.optional (stdenv.hostPlatform.uname.release != null)
              "-DCMAKE_SYSTEM_VERSION=${stdenv.hostPlatform.release}"
              ++ lib.optional (stdenv.buildPlatform.uname.system != null)
              "-DCMAKE_HOST_SYSTEM_NAME=${stdenv.buildPlatform.uname.system}"
              ++ lib.optional (stdenv.buildPlatform.uname.processor != null)
              "-DCMAKE_HOST_SYSTEM_PROCESSOR=${stdenv.buildPlatform.uname.processor}"
              ++ lib.optional (stdenv.buildPlatform.uname.release != null)
