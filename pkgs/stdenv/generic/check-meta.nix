...
let
  ...
  areLicenseListsValid = if lib.mutuallyExclusive whitelist blacklist then
    ...
    throw
    "whitelistedLicenses and blacklistedLicenses are not mutually exclusive.";

  hasLicense = attrs: attrs ? meta.license;

  hasWhitelistedLicense = assert areLicenseListsValid;
    attrs:
    hasLicense attrs && builtins.elem attrs.meta.license whitelist;

  hasBlacklistedLicense = assert areLicenseListsValid;
    attrs:
  ...
  hasAllowedInsecure = attrs:
    ...
    || allowInsecurePredicate attrs || builtins.getEnv "NIXPKGS_ALLOW_INSECURE"
    == "1";
  ...
  remediateOutputsToInstall = attrs:
    let
      ...
      missingOutputs =
        ...
        expectedOutputs;
    in ''
      The package ${attrs.name} has set meta.outputsToInstall to: ${
        builtins.concatStringsSep ", " expectedOutputs
      }

      however ${attrs.name} only has the outputs: ${
  ...
  handleEvalIssue = { meta, attrs }:
    ...
    let
      msg = if inHydra then
        ...
        ''
          Package ‘${attrs.name or "«name-missing»"}’ in ${
            pos_str meta
  ...
  checkValidity = attrs:
    if hasDeniedUnfreeLicense attrs && !(hasWhitelistedLicense attrs) then {
      ...
      reason = "blacklisted";
      errormsg =
        "has a blacklisted license (‘${showLicense attrs.meta.license}’)";
