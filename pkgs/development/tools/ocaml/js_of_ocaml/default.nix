...
let
  version =
    if stdenv.lib.versionAtLeast ocaml.version "4.02" then "2.8.4" else "2.7";
...
in stdenv.mkDerivation {
  ...
  buildInputs = [
    ...
    which
    base64
  ] ++ stdenv.lib.optionals (stdenv.lib.versionAtLeast ocaml.version "4.02") [
    yojson
    tyxml
  ];
  ...
  meta = with stdenv.lib; {
    ...
    description =
      "Compiler of OCaml bytecode to Javascript. It makes it possible to run Ocaml programs in a Web browser";
