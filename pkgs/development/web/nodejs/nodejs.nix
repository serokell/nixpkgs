{ stdenv, fetchurl, openssl, python2, zlib, libuv, utillinux, http-parser, pkgconfig, which
# Updater dependencies
, writeScript, coreutils, gnugrep, jq, curl, common-updater-scripts, nix, runtimeShell, gnupg, darwin, xcbuild, procps, icu
}:
...
let
  ...
  useSharedHttpParser = !stdenv.isDarwin
    && versionOlder "${majorVersion}.${minorVersion}" "11.4";

  sharedLibDeps = {
    inherit openssl zlib libuv;
  } // (optionalAttrs useSharedHttpParser { inherit http-parser; });
  ...
  copyLibHeaders = map (name: "${getDev sharedLibDeps.${name}}/include/*")
