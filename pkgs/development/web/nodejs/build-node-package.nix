...
# Node package name
pkgName ? if version != "" then
  stdenv.lib.removeSuffix "-${version}" name
else
...
let
  self = let
    ...
    mapDependencies = deps: f: rec {
      ...
      recursiveDeps =
        filterAttrs f (removeAttrs attrDeps (attrNames requiredDeps));
    };
    ...
    _peerDependencies =
      mapDependencies peerDependencies (name: dep: dep.pkgName != pkgName);

    requiredDependencies = _dependencies.requiredDeps
      // _optionalDependencies.requiredDeps // _peerDependencies.requiredDeps;

    recursiveDependencies = _dependencies.recursiveDeps
      // _optionalDependencies.recursiveDeps // _peerDependencies.recursiveDeps;
