...
let
  ...
  patches = [ ../use-source-date-epoch.patch ]
    ...
    ++ optional stdenv.hostPlatform.isMusl (fetchpatch {
      url =
        "https://raw.githubusercontent.com/richfelker/musl-cross-make/e84b1bd1fc12a3def33111ca6df522cd6e5ec361/patches/gcc-5.3.0/0001-musl.diff";
  ...
  crossMingw = targetPlatform != hostPlatform && targetPlatform.libc
    == "msvcrt";
  crossDarwin = targetPlatform != hostPlatform && targetPlatform.libc
    == "libSystem";
  crossConfigureFlags =
    ...
    [
      ...
      [
        (if crossDarwin then
          "--with-sysroot=${getLib libcCross}/share/sysroot"
        else
  ...
  crossNameAddon = if targetPlatform != hostPlatform then
...
stdenv.mkDerivation ({
  ...
  nativeBuildInputs = [ texinfo which gettext ]
    ++ (optional (perl != null) perl) ++ (optional javaAwtGtk pkgconfig);
  ...
  configureFlags =
    ...
    [
      ...
      "--enable-languages=${
        ...
        ++ optional langJava "java" ++ optional langGo "go"
        ++ optional langObjC "objc" ++ optional langObjCpp "obj-c++"
    ...
    (if (enableMultilib || targetPlatform.isAvr) then [
      ...
      [ "--disable-multilib" ]) ++ optional (!enableShared) "--disable-shared"
    ...
    optional (targetPlatform == hostPlatform && targetPlatform.isx86_32)
  ...
  buildFlags = optional (bootstrap && hostPlatform == buildPlatform)
  ...
  installTargets = if stripped then "install-strip" else "install";
  ...
  meta = {
    ...
    license =
      stdenv.lib.licenses.gpl3Plus; # runtime support libraries are typically LGPLv3+
    ...
    platforms = stdenv.lib.platforms.linux ++ stdenv.lib.platforms.freebsd
      ++ stdenv.lib.platforms.illumos ++ stdenv.lib.platforms.darwin;
