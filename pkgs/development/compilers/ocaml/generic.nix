{ minor_version, major_version, patch_version, url ? null, sha256, ... }@args:
let
...
in stdenv.mkDerivation (args // rec {
  ...
  configureFlags = optionals useX11
    (if stdenv.lib.versionAtLeast version "4.08" then [
      "--x-libraries=${x11lib}"
      "--x-includes=${x11inc}"
    ] else [
      "-x11lib"
  ...
  buildFlags = "world"
    + optionalString useNativeCompilers " bootstrap world.opt";
