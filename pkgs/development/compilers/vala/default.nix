{ stdenv, lib, fetchurl, fetchpatch, pkgconfig, flex, bison, libxslt, autoconf, autoreconfHook, graphviz, glib, libiconv, libintl, libtool, expat, substituteAll
}:

let
  generic = lib.makeOverridable
    ({ version, sha256, extraNativeBuildInputs ? [ ], extraBuildInputs ?
      [ ], withGraphviz ? false }:
    let
      ...
      graphvizPatch = let
        fp = { commit, sha256 }:
          fetchpatch {
            url =
              "https://github.com/openembedded/openembedded-core/raw/${commit}/meta/recipes-devtools/vala/vala/disable-graphviz.patch";
            inherit sha256;
          };
    ...
    in stdenv.mkDerivation rec {
      ...
      preBuild = lib.optional disableGraphviz
...
in rec {
  ...
  vala_0_38 = generic {
    ...
    extraNativeBuildInputs = [ autoconf ]
