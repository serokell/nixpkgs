...
in let
  result = stdenv.mkDerivation rec {
    name = if sourcePerArch.packageType == "jdk" then
      "adoptopenjdk-${sourcePerArch.vmType}-bin-${version}"
    else
      "adoptopenjdk-${sourcePerArch.packageType}-${sourcePerArch.vmType}-bin-${version}";
    ...
    postFixup = ''
      rpath+="''${rpath:+:}${
        stdenv.lib.concatStringsSep ":" (map (a: "$out/${a}") rSubPaths)
      }"
    ...
    meta = with stdenv.lib; {
      ...
      platforms = stdenv.lib.mapAttrsToList (arch: _: arch + "-linux")
