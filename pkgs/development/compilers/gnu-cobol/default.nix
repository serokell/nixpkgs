{ stdenv, fetchurl, gcc, makeWrapper, db, gmp, ncurses }:
...
in stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      "https://sourceforge.com/projects/open-cobol/files/gnu-cobol/${version}/gnucobol-${version}.tar.gz";
  ...
  cflags =
    stdenv.lib.concatMapStringsSep " " (p: "-L" + (lib.getLib p) + "/lib ")
    buildInputs;
  ldflags =
    stdenv.lib.concatMapStringsSep " " (p: "-I" + (lib.getDev p) + "/include ")
    buildInputs;
