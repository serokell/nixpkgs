{ stdenv, fetchurl, which, coq, coquelicot, flocq, mathcomp, bignums ? null }:

let
  params = if stdenv.lib.versionAtLeast coq.coq-version "8.7" then {
    version = "3.4.0";
    uid = "37524";
...
in stdenv.mkDerivation {
  ...
  src = fetchurl {
    url =
      "https://gforge.inria.fr/frs/download.php/file/${params.uid}/interval-${params.version}.tar.gz";
  ...
  configurePhase =
    "./configure --libdir=$out/lib/coq/${coq.coq-version}/user-contrib/Interval";
  buildPhase = "./remake";
