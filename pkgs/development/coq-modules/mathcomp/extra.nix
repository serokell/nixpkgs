{ stdenv, fetchFromGitHub, coq, ssreflect, coqPackages, recurseIntoAttrs }:
with builtins // stdenv.lib;
let current-ssreflect = ssreflect;
in let
  ...
  versions = {
    "1.9.0" = {
      ...
      analysis = {
        ...
        extra-deps = with coqPackages; [
          mathcomp_1_9-finmap
          mathcomp_1_9-bigenough
        ];
      };
      multinomials = {
        version = "1.3";
  ...
  packageGen = {
    ...
    let
      theCompatibleCoqVersions = if compatibleCoqVersions == null then
        ssreflect.compatibleCoqVersions
      else
        compatibleCoqVersions;
      mc-core-deps =
        if builtins.isNull core-deps then [ ssreflect ] else core-deps;
    in {
  ...
  select = x:
    mapAttrs (n: pkg: { package = n; } // pkg) (recursiveUpdate param x);

  for-version = v: suffix:
    (mapAttrs' (n: pkg: {
