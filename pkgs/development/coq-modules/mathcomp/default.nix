...
let
  ...
  mkMathcompGenFrom = o: l: mcv:
    fold (pkg: pkgs: pkgs // mkMathcompGen o pkg mcv) { } l;
  mkMathcompGen = overrides: mathcomp-pkg: mathcomp-version:
    let
      coq-version-check =
        mathcomp-coq-versions.${mathcomp-version} or (_: false);
      pkgpath = {
        single = "mathcomp";
      }.${mathcomp-pkg} or "mathcomp/${mathcomp-pkg}";
      pkgname = {
        single = "mathcomp";
      }.${mathcomp-pkg} or "mathcomp-${mathcomp-pkg}";
      pkgallMake = ''
  ...
  mathcompCorePkgs = recurseIntoAttrs (mapDerivationAttrset dontDistribute
    (mathcompGen default-mathcomp-version));
...
in rec {
  ...
  inherit mathcompGen mathcompCorePkgs mathcompCorePkgs_1_7 mathcompCorePkgs_1_8
    mathcompCorePkgs_1_9;
  ...
  name = "mathcomp-${n}";
  value = pkg;
}) mathcompCorePkgs) // (mapAttrs' (n: pkg: {
  name = "mathcomp-${n}_1_7";
  value = pkg;
}) mathcompCorePkgs_1_7) // (mapAttrs' (n: pkg: {
  name = "mathcomp-${n}_1_8";
  value = pkg;
}) mathcompCorePkgs_1_8) // (mapAttrs' (n: pkg: {
  name = "mathcomp-${n}_1_9";
  value = pkg;
}) mathcompCorePkgs_1_9)
