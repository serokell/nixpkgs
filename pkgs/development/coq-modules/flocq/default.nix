...
in stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      "https://gforge.inria.fr/frs/download.php/file/${params.uid}/flocq-${version}.tar.gz";
    inherit (params) sha256;
  };

  nativeBuildInputs = [ bash which autoconf automake ];
  buildInputs = [ coq ] ++ (with coq.ocamlPackages; [ ocaml camlp5 ]);
  ...
  passthru = {
    compatibleCoqVersions = v:
      builtins.elem v [ "8.5" "8.6" "8.7" "8.8" "8.9" ];
  };
}
