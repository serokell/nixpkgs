...
builtins.intersectAttrs super {
  ...
  al = appendConfigureFlag super.al
    "--extra-include-dirs=${pkgs.openal}/include/AL";

  # Won't find it's header files without help.
  sfml-audio = appendConfigureFlag super.sfml-audio
    "--extra-include-dirs=${pkgs.openal}/include/AL";
  ...
  haskakafka = overrideCabal super.haskakafka (drv: {
    preConfigure =
      "sed -i -e /extra-lib-dirs/d -e /include-dirs/d haskakafka.cabal";
  ...
  hw-kafka-client = dontCheck (overrideCabal super.hw-kafka-client (drv: {
    preConfigure =
      "sed -i -e /extra-lib-dirs/d -e /include-dirs/d -e /librdkafka/d hw-kafka-client.cabal";
    configureFlags = "--extra-include-dirs=${pkgs.rdkafka}/include/librdkafka";
  }));
  ...
  heist =
    overrideCabal super.heist (drv: { testToolDepends = [ pkgs.pandoc ]; });

  # the system-fileio tests use canonicalizePath, which fails in the sandbox
  system-fileio = if pkgs.stdenv.isDarwin then
    dontCheck super.system-fileio
  else
    super.system-fileio;
  ...
  x509-system =
    if pkgs.stdenv.hostPlatform.isDarwin && !pkgs.stdenv.cc.nativeLibc then
  ...
  gio = disableHardening (addPkgconfigDepend
    (addBuildTool super.gio self.buildHaskellPackages.gtk2hs-buildtools)
    pkgs.glib) [ "fortify" ];
  glib = disableHardening (addPkgconfigDepend
    (addBuildTool super.glib self.buildHaskellPackages.gtk2hs-buildtools)
    pkgs.glib) [ "fortify" ];
  gtk3 = disableHardening (super.gtk3.override { inherit (pkgs) gtk3; })
    [ "fortify" ];
  gtk = disableHardening (addPkgconfigDepend
    (addBuildTool super.gtk self.buildHaskellPackages.gtk2hs-buildtools)
    pkgs.gtk2) [ "fortify" ];
  ...
  hasql =
    dontCheck super.hasql; # http://hydra.cryp.to/build/502489/nixlog/4/raw
  hasql-transaction =
    dontCheck super.hasql-transaction; # wants to connect to postgresql
  hjsonschema =
    overrideCabal super.hjsonschema (drv: { testTarget = "local"; });
  ...
  mime-mail = appendConfigureFlag super.mime-mail
    ''--ghc-option=-DMIME_MAIL_SENDMAIL_PATH="sendmail"'';

  # Help the test suite find system timezone data.
  tz = overrideCabal super.tz
  ...
  wxc =
    (addBuildDepend super.wxc self.split).override { wxGTK = pkgs.wxGTK30; };
  ...
  foundation = if pkgs.stdenv.isDarwin then
    dontCheck super.foundation
  else
    super.foundation;
  ...
  llvm-general =
    ...
    in if !pkgs.stdenv.isDarwin then
      ...
      overrideCabal base (drv: {
        preConfigure = ''
          sed -i llvm-general.cabal \
              -e 's,extra-libraries: stdc++,extra-libraries: c++,'
        '';
        configureFlags = (drv.configureFlags or [ ])
          ++ [ "--extra-include-dirs=${pkgs.libcxx}/include/c++/v1" ];
        librarySystemDepends = [ pkgs.libcxx ]
  ...
  yesod-bin = if pkgs.stdenv.isDarwin then
    addBuildDepend super.yesod-bin pkgs.darwin.apple_sdk.frameworks.Cocoa
  else
    super.yesod-bin;

  hmatrix = if pkgs.stdenv.isDarwin then
    addBuildDepend super.hmatrix pkgs.darwin.apple_sdk.frameworks.Accelerate
  else
    super.hmatrix;
  ...
  epic = addExtraLibraries
    (addBuildTool super.epic self.buildHaskellPackages.happy) [
      pkgs.boehmgc
      pkgs.gmp
    ];
  ...
  miniball = overrideCabal super.miniball
    (drv: { librarySystemDepends = [ pkgs.miniball ]; });
  SDL-image = overrideCabal super.SDL-image (drv: {
    librarySystemDepends = [ pkgs.SDL pkgs.SDL_image ]
      ++ drv.librarySystemDepends or [ ];
  });
  SDL-ttf = overrideCabal super.SDL-ttf
    (drv: { librarySystemDepends = [ pkgs.SDL pkgs.SDL_ttf ]; });
  SDL-mixer = overrideCabal super.SDL-mixer
    (drv: { librarySystemDepends = [ pkgs.SDL pkgs.SDL_mixer ]; });
  SDL-gfx = overrideCabal super.SDL-gfx
    (drv: { librarySystemDepends = [ pkgs.SDL pkgs.SDL_gfx ]; });
  ...
  ${if pkgs.stdenv.isDarwin then null else "GLUT"} =
    addPkgconfigDepend (appendPatch super.GLUT ./patches/GLUT.patch)
    pkgs.freeglut;
  ...
  fsnotify = if pkgs.stdenv.isDarwin then
    addBuildDepend (dontCheck super.fsnotify)
  ...
  secp256k1 = addBuildTools super.secp256k1 [
  ...
  haskell-gi-base =
    addBuildDepend super.haskell-gi-base pkgs.gobject-introspection;
  ...
  servant-client-core =
    if (pkgs.lib.getVersion super.servant-client-core) == "0.15" then
      appendPatch super.servant-client-core
      ./patches/servant-client-core-streamBody.patch
    else
  ...
  futhark = if pkgs.stdenv.isDarwin then
    super.futhark
  else
    with pkgs;
