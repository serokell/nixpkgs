...
let
  ...
  withPackages = packages:
    buildPackages.callPackage ./with-packages-wrapper.nix {
  ...
  all-cabal-hashes-component = name: version:
    pkgs.runCommand "all-cabal-hashes-component-${name}-${version}" { } ''
...
in package-set { inherit pkgs stdenv callPackage; } self // {
  ...
  callCabal2nixWithOptions = name: src: extraCabal2nixOptions: args:
    let
      filter = path: type:
        pkgs.lib.hasSuffix "${name}.cabal" path || baseNameOf path
        == "package.yaml";
    ...
    in overrideCabal (callPackageKeepDeriver expr args)
    (orig: { inherit src; });
  ...
  shellFor = { packages, withHoogle ? false, ... }@args:
    let
      ...
      name = if pkgs.lib.length selected == 1 then
        "ghc-shell-for-${(pkgs.lib.head selected).name}"
      else
        "ghc-shell-for-packages";
      ...
      withPackages =
        if withHoogle then self.ghcWithHoogle else self.ghcWithPackages;
    ...
    in pkgs.stdenv.mkDerivation (mkDrvArgs // {
      ...
      LANG = "en_US.UTF-8";
      LOCALE_ARCHIVE =
        pkgs.lib.optionalString (stdenv.hostPlatform.libc == "glibc")
        "${buildPackages.glibcLocales}/lib/locale/locale-archive";
      ...
      "NIX_${ghcCommandCaps}_LIBDIR" = if ghc.isHaLVM or false then
        ...
        "${ghcEnv}/lib/${ghcCommand}-${ghc.version}";
    });
