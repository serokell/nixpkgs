...
let
  ...
  x_ignore_nofocus = fetchFromGitHub {
    owner = "SeleniumHQ";
    repo = "selenium";
    rev = "selenium-3.6.0";
...
in buildPythonPackage rec {
  ...
  propagatedBuildInputs = [ geckodriver urllib3 ];
  ...
  patchPhase = stdenv.lib.optionalString stdenv.isLinux ''
    ...
    cc -shared \
      ...
      x_ignore_nofocus.o
    cp -v x_ignore_nofocus.so selenium/webdriver/firefox/${
      if stdenv.is64bit then "amd64" else "x86"
    }/
  '';
