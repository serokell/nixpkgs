...
in buildPythonPackage rec {
  ...
  patches = [
    ...
    (fetchurl {
      url =
        "https://gitlab.freedesktop.org/gstreamer/gst-python/commit/d64bbc1e0c3c948c148f505cc5f856ce56732880.diff";
      sha256 = "1n9pxmcl1x491mp47avpcw2a6n71lm0haz6mfas168prkgsk8q3r";
    })
    # Fixes python2 build from the above changes
    (fetchurl {
      url =
        "https://gitlab.freedesktop.org/gstreamer/gst-python/commit/f79ac2d1434d7ba9717f3e943cfdc76e121eb5dd.diff";
  ...
  postPatch = ''
    substituteInPlace meson.build --replace python3 python${
      if isPy3k then "3" else "2"
    }
  '';

  nativeBuildInputs =
    [ meson ninja pkgconfig python gobject-introspection gst-plugins-base ];
