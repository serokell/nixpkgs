...
&& stdenv.lib.versionAtLeast ocaml.version "4.03" then
  throw "uri${
    stdenv.lib.optionalString legacyVersion "_p4"
  } is not available for OCaml ${ocaml.version}"
else

  with if legacyVersion then {
    version = "1.9.1";
  ...
  stdenv.mkDerivation {
    ...
    propagatedBuildInputs =
      [ re (if legacyVersion then sexplib_p4 else sexplib) stringext ];
