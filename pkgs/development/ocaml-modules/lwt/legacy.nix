...
|| stdenv.lib.versionAtLeast ocaml.version "4.06" then
  throw "lwt is not available for OCaml ${ocaml.version}"
else
  ...
  in let
    optionals = stdenv.lib.optionals (!stdenv.lib.versionAtLeast version "3");
  ...
  in buildOcaml rec {
    ...
    buildInputs =
      [ which cryptopp ocaml findlib ocamlbuild glib ncurses camlp4 cppo ]
      ++ stdenv.lib.optional ppxSupport ppx_tools;

    propagatedBuildInputs = [ result ] ++ optionals [ react ssl ] ++ [ libev ];
    ...
    meta = with stdenv.lib; {
      ...
      maintainers = with maintainers; [ z77z vbgl gal_bolle ];
    };
  }
