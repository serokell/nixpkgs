...
in assert versionAtLeast (getVersion ocaml) "4.00.0";
assert versionAtLeast (getVersion findlib) "1.3.3";

if versionAtLeast ocaml.version "4.06" then
  throw "bolt is not available for OCaml ${ocaml.version}"
else
  ...
  stdenv.mkDerivation rec {
    ...
    patches = [
      (fetchpatch {
        url =
          "https://raw.githubusercontent.com/ocaml/opam-repository/master/packages/bolt/bolt.1.4/files/opam.patch";
