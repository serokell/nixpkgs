...
if !stdenv.lib.versionAtLeast ocaml.version "4.03" then
  ...
  let mkpath = p: n: "${p}/lib/ocaml/${ocaml.version}/site-lib/${n}";
  ...
  in stdenv.mkDerivation rec {
    ...
    propagatedBuildInputs =
      [ cryptokit ipaddr lwt_log lwt_ssl ocamlnet ocaml_pcre tyxml xml-light ];
    ...
    postFixup = ''
      ...
      wrapProgram $out/bin/ocsigenserver \
        --prefix CAML_LD_LIBRARY_PATH : "${mkpath ssl "ssl"}:${
        mkpath ocamlnet "netsys"
        }:${mkpath ocamlnet "netstring"}:${mkpath ocaml_pcre "pcre"}:${
          mkpath cryptokit "cryptokit"
