...
let withLwt = lwt != null;

in if !versionAtLeast ocaml.version "4.04" then
  throw "tls is not available for OCaml ${ocaml.version}"
else
  ...
  stdenv.mkDerivation rec {
    ...
    buildPhase =
      "${topkg.run} build --tests true --with-mirage false --with-lwt ${
        if withLwt then "true" else "false"
      }";
