...
{ pkgs, lib, lua, overrides ? (self: super: { }) }:
...
let
  ...
  initialPackages =
    (pkgs.callPackage ../../top-level/lua-packages.nix { inherit lua; });
  ...
  extensible-self = lib.makeExtensible (extends overrides
    (extends overridenPackages (extends generatedPackages initialPackages)));
in extensible-self
