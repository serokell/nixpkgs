...
with super; {
  ...
  lgi = super.lgi.override ({
    ...
    buildInputs = [ pkgs.glib pkgs.gobjectIntrospection ];
    patches = [
      (pkgs.fetchpatch {
  ...
  lrexlib-gnu = super.lrexlib-gnu.override ({ buildInputs = [ pkgs.gnulib ]; });
  ...
  lrexlib-posix =
    super.lrexlib-posix.override ({ buildInputs = [ pkgs.glibc.dev ]; });

  ltermbox = super.ltermbox.override ({ disabled = !isLua51 || isLuaJIT; });

  lua-iconv = super.lua-iconv.override ({ buildInputs = [ pkgs.libiconv ]; });
  ...
  luadbi-mysql = super.luadbi-mysql.override ({
    ...
    '';
    buildInputs = [ pkgs.mysql.client pkgs.mysql.connector-c ];
  });

  luadbi-postgresql =
    super.luadbi-postgresql.override ({ buildInputs = [ pkgs.postgresql ]; });
  ...
  luasystem = super.luasystem.override ({ buildInputs = [ pkgs.glibc ]; });

  luazip = super.luazip.override ({ buildInputs = [ pkgs.zziplib ]; });
