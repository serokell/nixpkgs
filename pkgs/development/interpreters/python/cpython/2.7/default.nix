...
let
  ...
  patches = [ # Look in C_INCLUDE_PATH and LIBRARY_PATH for stuff.
    ...
    [ ./use-correct-tcl-tk-on-darwin.patch ] ++ optionals stdenv.isLinux [
  ...
  configureFlags = [
    ...
    [ "--with-system-ffi" ] ++ optionals stdenv.hostPlatform.isCygwin [
      "--with-system-expat"
  ...
  buildInputs = optional (stdenv ? cc && stdenv.cc.libc != null) stdenv.cc.libc
    ++ [ bzip2 openssl zlib ]
    ++ optional (stdenv.hostPlatform.isCygwin || stdenv.hostPlatform.isAarch64)
    libffi ++ optional stdenv.hostPlatform.isCygwin expat
