...
let
  dsoPatch51 = fetchurl {
    url =
      "https://projects.archlinux.org/svntogit/packages.git/plain/trunk/lua-arch.patch?h=packages/lua51";
  ...
  dsoPatch52 = fetchurl {
    url =
      "https://projects.archlinux.org/svntogit/packages.git/plain/trunk/liblua.so.patch?h=packages/lua52";
...
in rec {
  ...
  lua5_2 = callPackage ./interpreter.nix {
    ...
    patches =
      if stdenv.isDarwin then [ ./5.2.darwin.patch ] else [ dsoPatch52 ];
  };
  ...
  lua5_1 = callPackage ./interpreter.nix {
    ...
    patches =
      (if stdenv.isDarwin then [ ./5.1.darwin.patch ] else [ dsoPatch51 ])
