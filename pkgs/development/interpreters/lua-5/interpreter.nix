...
let
  ...
  self = stdenv.mkDerivation rec {
    ...
    setupHook =
      luaPackages.lua-setup-hook LuaPathSearchPaths LuaCPathSearchPaths;
    ...
    makeFlags = [
      ...
      "LDFLAGS=-fPIC"
      "V=${luaversion}"
    ] ++ (if stdenv.isDarwin then [ "PLAT=macosx" ] else [ "PLAT=linux" ]);

    configurePhase = ''
      runHook preConfigure

      makeFlagsArray+=(CFLAGS="-DLUA_USE_LINUX -O2 -fPIC${
        if compat then " -DLUA_COMPAT_ALL" else ""
      }" )
      makeFlagsArray+=(${
        stdenv.lib.optionalString stdenv.isDarwin ''CC="$CC"''
      })
