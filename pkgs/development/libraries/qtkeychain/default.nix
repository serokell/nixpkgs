...
stdenv.mkDerivation rec {
  ...
  version =
    "0.9.1"; # verify after nix-build with `grep -R "set(PACKAGE_VERSION " result/`
  ...
  nativeBuildInputs = [ cmake ] ++ stdenv.lib.optional (!stdenv.isDarwin)
    [ pkgconfig ] # for finding libsecret
  ;
