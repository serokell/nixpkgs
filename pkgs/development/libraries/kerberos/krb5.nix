...
stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      "https://kerberos.org/dist/krb5/${majorVersion}/krb5-${version}.tar.gz";
  ...
  nativeBuildInputs = [ pkgconfig perl ] ++ optional (!libOnly) yacc
    # Provides the mig command used by the build scripts
    ++ optional stdenv.isDarwin bootstrap_cmds;

  buildInputs = [ openssl ] ++ optionals (stdenv.hostPlatform.isLinux
