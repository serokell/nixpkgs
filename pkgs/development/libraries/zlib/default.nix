{ stdenv, fetchurl, static ? true, shared ? true }:
...
stdenv.mkDerivation (rec {
  ...
  patches = stdenv.lib.optional stdenv.hostPlatform.isCygwin
  ...
  outputs = [ "out" "dev" ] ++ stdenv.lib.optional (shared && static) "static";
  ...
  makeFlags = [ "PREFIX=${stdenv.cc.targetPrefix}" ]
    ++ stdenv.lib.optionals (stdenv.hostPlatform.libc == "msvcrt") [
      "-f"
      "win32/Makefile.gcc"
    ] ++ stdenv.lib.optionals shared [ "SHARED_MODE=1" ];
