...
coreutils, bison, flex, gdb, gperf, lndir, perl, pkgconfig, python2, which,
# darwin support
darwin, libiconv,
...
let compareVersion = v: builtins.compareVersions version v;
...
in stdenv.mkDerivation {
  ...
  nativeBuildInputs = [ bison flex gperf lndir perl pkgconfig python2 which ];
  ...
  NIX_CFLAGS_COMPILE = [
    ...
    ''
      -D${
        if compareVersion "5.11.0" >= 0 then
          "LIBRESOLV_SO"
        else
          "NIXPKGS_LIBRESOLV"
      }="${stdenv.cc.libc.out}/lib/libresolv"''
    ''-DNIXPKGS_LIBXCURSOR="${libXcursor.out}/lib/libXcursor"''
  ]

    ++ lib.optional libGLSupported
    ''-DNIXPKGS_MESA_GL="${libGL.out}/lib/libGL"'' ++ lib.optionals withGtk3 [
      ''
  ...
  configureFlags = [
    ...
    "-icu"
    "-pch"
  ] ++ lib.optionals (compareVersion "5.11.0" < 0) [ "-qml-debug" ]
    ++ lib.optionals (compareVersion "5.9.0" < 0) [
      ...
      lib.optionals (compareVersion "5.9.0" >= 0) {
        ...
        "westmere" =
          [ "-sse2" "-sse3" "-ssse3" "-sse4.1" "-sse4.2" "-no-avx" "-no-avx2" ];
        "sandybridge" =
          [ "-sse2" "-sse3" "-ssse3" "-sse4.1" "-sse4.2" "-avx" "-no-avx2" ];
        "ivybridge" =
          [ "-sse2" "-sse3" "-ssse3" "-sse4.1" "-sse4.2" "-avx" "-no-avx2" ];
        "haswell" =
