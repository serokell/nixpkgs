...
in qtModule {
  ...
  postPatch =
    ...
    + optionalString (lib.versionOlder qtCompatVersion "5.12") ''
      ...
      substituteInPlace ./src/3rdparty/chromium/v8/${
        if qt56 then "build" else "gypfiles"
      }/toolchain.gypi \
  ...
  NIX_CFLAGS_COMPILE =
    ...
    [ "-march=westmere" ] ++ lib.optionals stdenv.isDarwin [
  ...
  propagatedBuildInputs = [
    ...
    (stdenv.hostPlatform.isAarch32 || stdenv.hostPlatform.isAarch64) [ ffmpeg ]
  ...
  buildInputs = optionals stdenv.isDarwin (with darwin; [
    ...
    (runCommand "MacOS_SDK_sandbox.h" { } ''
      ...
      }"/include/sandbox.h "$out"/include/sandbox.h
    '')
  ]);
