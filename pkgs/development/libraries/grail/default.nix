{ enableX11 ? true, stdenv, fetchurl, pkgconfig, xorg, python3, frame }:
...
stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      "https://launchpad.net/grail/trunk/${version}/+download/${name}.tar.bz2";
    sha256 = "1wwx5ibjdz5pyd0f5cd1n91y67r68dymxpm2lgd829041xjizvay";
  };

  nativeBuildInputs = [ pkgconfig ];
  buildInputs = [ python3 frame ] ++ stdenv.lib.optionals enableX11 [
    xorg.libX11
    xorg.libXtst
    xorg.libXext
    xorg.libXi
