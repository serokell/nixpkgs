...
in stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      "http://archive.eclipse.org/eclipse/downloads/drops4/R-${fullVersion}/${name}-${metadata.platform}.zip";
  ...
  buildInputs =
    [ jdk gtk2 libXt libXtst libXi libGLU_combined webkitgtk libsoup ];

  NIX_LFLAGS =
    (map (x: "-L${lib.getLib x}/lib") [ xorg.libX11 pango gdk_pixbuf glib ])
  ...
  meta = with stdenv.lib; {
    ...
    description =
      "An widget toolkit for Java to access the user-interface facilities of the operating systems on which it is implemented";
