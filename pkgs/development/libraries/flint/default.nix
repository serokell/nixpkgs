...
stdenv.mkDerivation rec {
  ...
  buildInputs = [ gmp mpir mpfr ntl ]
    ++ stdenv.lib.optionals withBlas [ openblas ];
  ...
  configureFlags = [
    ...
    "--with-mpfr=${mpfr}"
    "--with-ntl=${ntl}"
  ] ++ stdenv.lib.optionals withBlas [ "--with-blas=${openblas}" ];
  ...
  patches = [
    (fetchpatch {
      ...
      url =
        "https://github.com/wbhart/flint2/commit/49fbcd8f736f847d3f9667f9f7d5567ef4550ecb.patch";
