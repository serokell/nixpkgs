...
stdenv.mkDerivation rec {
  name =
    let postfix = if gtkVersion == null then "glib" else "gtk${gtkVersion}";
  ...
  src = fetchurl {
    url =
      "${meta.homepage}/${versionMajor}/${version}/+download/libdbusmenu-${version}.tar.gz";
  ...
  buildInputs =
    [ glib dbus-glib json-glib gobject-introspection gnome-doc-utils ]
