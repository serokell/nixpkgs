...
let
  nssPEM = fetchurl {
    url =
      "http://dev.gentoo.org/~polynomial-c/mozilla/nss-3.15.4-pem-support-20140109.patch.xz";
...
in stdenv.mkDerivation rec {
  ...
  src = fetchurl {
    url =
      "mirror://mozilla/security/nss/releases/NSS_${underscoreVersion}_RTM/src/${name}.tar.gz";
  ...
  makeFlags = let cpu = stdenv.hostPlatform.parsed.cpu.name;
    in [
