...
stdenv.mkDerivation rec {
  ...
  patches = [
    ...
    (fetchpatch {
      url =
        "https://raw.githubusercontent.com/gentoo/gentoo/7c5457e265bd40c156a8fe6b2ff94a4e34bcea8e/net-libs/webkit-gtk/files/webkit-gtk-2.4.9-gcc-6.patch";
    ...
    ./libc++.patch
    ./plugin-none.patch
  ] ++ optionals stdenv.hostPlatform.isMusl [ ./fix-execinfo.patch ];
  ...
  configureFlags = with stdenv.lib;
    ...
    ] ++ optional withGtk2 [ "--with-gtk=2.0" ]
    ++ optionals (withGtk2 || stdenv.isDarwin) [ "--disable-webkit2" ]
    ++ optionals stdenv.isDarwin [
      "--disable-x11-target"
    ...
    ++ optionals (!enableCredentialStorage) [ "--disable-credential-storage" ];

  NIX_CFLAGS_COMPILE = [ "-DU_NOEXCEPT=" "-Wno-expansion-to-defined" ];
  ...
  buildInputs = [
    ...
    xorg.libXt
    xorg.libXdamage
  ] ++ optionals enableCredentialStorage [ libsecret ]
    ++ (if stdenv.isDarwin then [ readline libedit libobjc ] else [ wayland ]);
