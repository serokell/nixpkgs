...
, blas64 ? null, buildPackages
# Select a specific optimization target (other than the default)
# See https://github.com/xianyi/OpenBLAS/blob/develop/TargetList.txt
, target ? null, enableStatic ? false }:
...
in let
  blas64 = if blas64_ != null then
    ...
    hasPrefix "x86_64" stdenv.hostPlatform.system;
  ...
  mkMakeFlagsFromConfig =
    mapAttrsToList (var: val: "${var}=${mkMakeFlagValue val}");
in stdenv.mkDerivation rec {
  ...
  nativeBuildInputs =
    [ perl which buildPackages.gfortran buildPackages.stdenv.cc ];
