...
let
  version = "12.1.51";
  isLinux = (stdenv.hostPlatform.system == "x86_64-linux"
...
!= "i686-linux") then
  ...
  stdenv.mkDerivation {
    ...
    src = if stdenv.hostPlatform.system == "x86_64-linux" then
      fetchurl {
        url =
          "https://developer.spotify.com/download/libspotify/libspotify-${version}-Linux-x86_64-release.tar.gz";
        sha256 = "0n0h94i4xg46hfba95n3ypah93crwb80bhgsg00f6sms683lx8a3";
      }
    else if stdenv.hostPlatform.system == "x86_64-darwin" then
      fetchurl {
        url =
          "https://developer.spotify.com/download/libspotify/libspotify-${version}-Darwin-universal.zip";
        sha256 = "1gcgrc8arim3hnszcc886lmcdb4iigc08abkaa02l6gng43ky1c0";
      }
    else if stdenv.hostPlatform.system == "i686-linux" then
      fetchurl {
        url =
          "https://developer.spotify.com/download/libspotify/libspotify-${version}-Linux-i686-release.tar.gz";
    ...
    # darwin-specific
    buildInputs =
      stdenv.lib.optional (stdenv.hostPlatform.system == "x86_64-darwin") unzip;

    # linux-specific
    installFlags = stdenv.lib.optionalString (isLinux) "prefix=$(out)";
    patchPhase = stdenv.lib.optionalString (isLinux)
      "${gnused}/bin/sed -i 's/ldconfig//' Makefile";
    postInstall = stdenv.lib.optionalString (isLinux) "mv -v share $out";

    passthru = {
      samples = if apiKey == null then
        throw ''
