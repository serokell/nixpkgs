...
in stdenv.mkDerivation rec {
  ...
  buildInputs = with stdenv;
    [ gfortran zlib ] ++ lib.optionals isLinux [ libnl numactl ]
    ++ [ libevent hwloc ] ++ lib.optional (isLinux || isFreeBSD) rdma-core;

  nativeBuildInputs = [ perl ];

  configureFlags = with stdenv;
    [ "--disable-mca-dso" ] ++ lib.optional isLinux "--with-libnl=${libnl.dev}"
