{ stdenv, lib, fetchurl, fetchpatch, substituteAll, libXrender, libXinerama, libXcursor, libXv, libXext, libXfixes, libXrandr, libSM, freetype, fontconfig, zlib, libjpeg, libpng, libmng, which, libGLU, openssl, dbus, cups, pkgconfig, libtiff, glib, icu, mysql, postgresql, sqlite, perl, coreutils, libXi, buildMultimedia ?
  stdenv.isLinux, alsaLib, gstreamer, gst-plugins-base, buildWebkit ?
    (stdenv.isLinux || stdenv.isDarwin), libGLSupported ?
      stdenv.lib.elem stdenv.hostPlatform.system
      stdenv.lib.platforms.mesaPlatforms, flashplayerFix ?
        false, gdk_pixbuf, gtkStyle ? stdenv.hostPlatform
          == stdenv.buildPlatform, gtk2, gnomeStyle ?
            false, libgnomeui, GConf, gnome_vfs, developerBuild ? false, docs ?
              false, examples ? false, demos ? false
                # darwin support
...
in stdenv.mkDerivation rec {
  ...
  patches = lib.optionals (stdenv.hostPlatform == stdenv.buildPlatform) [
    ...
    (fetchpatch {
      name = "qt4-gcc6.patch";
      url =
        "https://git.archlinux.org/svntogit/packages.git/plain/trunk/qt4-gcc6.patch?h=packages/qt4&id=ca773a144f5abb244ac4f2749eeee9333cac001f";
  ...
  # Qt doesn't directly need GLU (just GL), but many apps use, it's small and doesn't remain a runtime-dep if not used
    ...
    ++ lib.optionals (buildWebkit || buildMultimedia) [
      gstreamer
      gst-plugins-base
    ];
