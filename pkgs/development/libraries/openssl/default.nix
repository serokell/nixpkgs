...
let
  common = { version, sha256, patches ? [ ], withDocs ? false }:
    stdenv.mkDerivation rec {
      ...
      '' + optionalString
        (versionAtLeast version "1.1.0" && stdenv.hostPlatform.isMusl) ''
      ...
      == stdenv.buildPlatform then
        ...
        "./Configure mingw${
          optionalString (stdenv.hostPlatform.parsed.cpu.bits != 32)
      ...
      ] ++ stdenv.lib.optional enableSSL2 "enable-ssl2" ++ stdenv.lib.optional
        (versionAtLeast version "1.1.0" && stdenv.hostPlatform.isAarch64)
        "no-afalgeng";
  ...
  openssl_1_0_2 = common {
    ...
    patches = [
      ...
      (if stdenv.hostPlatform.isDarwin then
        ./1.0.2/use-etc-ssl-certs-darwin.patch
      else
        ./1.0.2/use-etc-ssl-certs.patch)
    ];
  };
