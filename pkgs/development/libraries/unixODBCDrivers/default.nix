...

{
  ...
  mariadb = stdenv.mkDerivation rec {
    ...
    src = fetchurl {
      url =
        "https://downloads.mariadb.org/interstitial/connector-odbc-${version}/src/${name}-ga-src.tar.gz";
  ...
  mysql = stdenv.mkDerivation rec {
    ...
    src = fetchurl {
      url =
        "https://dev.mysql.com/get/Downloads/Connector-ODBC/${majorVersion}/${name}-src.tar.gz";
  ...
  msodbcsql17 = stdenv.mkDerivation rec {
    ...
    src = fetchurl {
      url =
        "https://packages.microsoft.com/debian/9/prod/pool/main/m/msodbcsql17/msodbcsql${versionMajor}_${version}_amd64.deb";
    ...
    installPhase = ''
      ...
      ln -s ${
        lib.getLib openssl
      }/lib/libcrypto.so.1.0.0 $out/lib/libcrypto.so.1.0.2
