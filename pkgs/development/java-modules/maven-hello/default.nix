...
let poms = import ../poms.nix { inherit fetchMaven; };
in rec {
  mavenHelloRec =
    { mavenDeps, sha512, version, skipTests ? true, quiet ? true }:
    mavenbuild rec {
  ...
  mavenHello_1_1 = mavenHelloRec {
    mavenDeps = [ junit_4_12 mavenSurefireJunit4_2_12_4 hamcrestCore_1_3 ]
