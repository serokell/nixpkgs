...
let
  ...
  makeSystemTarball = { module, maintainers ? [ "viric" ], system }:
    ...
    in tarball // {
      meta = {
        description =
          "NixOS system tarball for ${system} - ${stdenv.hostPlatform.platform.name}";
  ...
  makeNetboot = { module, system, ... }:
    let
      ...
      kernelTarget =
        configEvaled.pkgs.stdenv.hostPlatform.platform.kernelTarget;
    in pkgs.symlinkJoin {
      name = "netboot";
...
in rec {
  ...
  netboot = forMatchingSystems [ "x86_64-linux" "aarch64-linux" ] (system:
    makeNetboot {
  ...
  iso_graphical = forMatchingSystems [ "x86_64-linux" ] (system:
    makeIso {
  ...
  sd_image_new_kernel = forMatchingSystems [ "aarch64-linux" ] (system:
    makeSdImage {
      module = {
        aarch64-linux =
          ./modules/installer/cd-dvd/sd-image-aarch64-new-kernel.nix;
      }.${system};
      type = "minimal-new-kernel";
      inherit system;
    });
