...
let
  ...
  componentServices = mkServices (name: attrs:
    let
      ...
      additionalAfter = firstServicesNoSelf
        ++ map mkSrvName (attrs.after or [ ]);
    ...
    systemd.services = {
      pkcs11-daemon = {
        ...
        serviceConfig.ExecStart =
          let softhsmLib = "${softhsm}/lib/softhsm/libsofthsm.so";
