...
, pkgs ? import ../.. { inherit system config; }, enableUnfree ? false
...
let
  ...
  mkElkTest = name: elk:
    let
      elasticsearchGe7 = builtins.compareVersions elk.elasticsearch.version "7"
        >= 0;
    in makeTest {
      inherit name;
