  ...
  testScriptFun =
    ...
    let
      ...
      hdFlags = ''
        ...
        + optionalString isEfi (if pkgs.stdenv.isAarch64 then
          ''bios => "${pkgs.OVMF.fd}/FV/QEMU_EFI.fd", ''
        else
  ...
  makeInstallerTest = name:
    ...
    makeTest {
      ...
      testScript = testScriptFun {
        inherit bootLoader createPartitions preBootCommands grubVersion
          grubDevice grubIdentifier grubUseEfi extraConfig testCloneConfig;
      };
    };
...
in {
  ...
  simpleClone = makeInstallerTest "simpleClone"
    (simple-test-config // clone-test-extraconfig);
  ...
  simpleUefiGrubClone = makeInstallerTest "simpleUefiGrubClone"
    (simple-uefi-grub-config // clone-test-extraconfig);
