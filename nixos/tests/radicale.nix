let
  ...
  common = { pkgs, ... }: {
    ...
    environment.etc."radicale/htpasswd".source =
      pkgs.runCommand "htpasswd" { } ''
        ${pkgs.apacheHttpd}/bin/htpasswd -bcB "$out" ${user} ${password}
      '';
  };

in import ./make-test.nix ({ lib, ... }@args: {
  name = "radicale";
  ...
  nodes = rec {
    ...
    radicale2 =
      lib.recursiveUpdate (common args) { system.stateVersion = "17.09"; };
  };
  ...
  testScript = { nodes }:
    let
      switchToConfig = nodeName:
        let newSystem = nodes.${nodeName}.config.system.build.toplevel;
