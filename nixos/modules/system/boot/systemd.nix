  ...
  serviceConfig = { name, config, ... }: {
    config = mkMerge [
      { # Default path for systemd services.  Should be quite minimal.
        path =
          [ pkgs.coreutils pkgs.findutils pkgs.gnugrep pkgs.gnused systemd ];
...
in {
  ...
  options = {
    ...
    systemd.targets = mkOption {
      default = { };
      type = with types;
        attrsOf (submodule [ { options = targetOptions; } unitConfig ]);
    ...
    systemd.services = mkOption {
      ...
      type = with types;
        attrsOf
        (submodule [ { options = serviceOptions; } unitConfig serviceConfig ]);
    ...
    systemd.sockets = mkOption {
      default = { };
      type = with types;
        attrsOf (submodule [ { options = socketOptions; } unitConfig ]);
    ...
    systemd.timers = mkOption {
      default = { };
      type = with types;
        attrsOf (submodule [ { options = timerOptions; } unitConfig ]);
    ...
    systemd.paths = mkOption {
      default = { };
      type = with types;
        attrsOf (submodule [ { options = pathOptions; } unitConfig ]);
    ...
    systemd.mounts = mkOption {
      ...
      type = with types;
        listOf
        (submodule [ { options = mountOptions; } unitConfig mountConfig ]);
      description = ''
    ...
    systemd.slices = mkOption {
      default = { };
      type = with types;
        attrsOf (submodule [ { options = sliceOptions; } unitConfig ]);
  ...
  config = {
    ...
    environment.etc = let
      ...
      generators = pkgs.runCommand "system-generators" {
        ...
        ${concatStrings (mapAttrsToList (generator: target: ''
