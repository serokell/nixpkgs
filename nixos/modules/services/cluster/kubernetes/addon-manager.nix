...
in {
  ...
  options.services.kubernetes.addonManager = with lib.types; {
    ...
    bootstrapAddonsKubeconfig =
      top.lib.mkKubeConfigOptions "Kubernetes addon manager bootstrap";
  };
  ...
  config = let
    ...
    in mkIf cfg.enable {
      ...
      services.kubernetes.addonManager.kubeconfig.server =
        mkDefault top.apiserverAddress;

      systemd.services.kube-addon-manager-bootstrap =
        mkIf (top.apiserver.enable && top.addonManager.bootstrapAddons != { }) {
          wantedBy = [ "kube-control-plane-online.target" ];
          after = [ "kube-apiserver.service" ];
          before = [ "kube-control-plane-online.target" ];
          path = [ pkgs.kubectl ];
          environment = {
            KUBECONFIG = top.lib.mkKubeConfig "kube-addon-manager-bootstrap"
              cfg.bootstrapAddonsKubeconfig;
          };
          preStart = with pkgs;
            let
              files =
      ...
      services.kubernetes.addonManager.bootstrapAddonsKubeconfig.server =
        mkDefault top.apiserverAddress;
