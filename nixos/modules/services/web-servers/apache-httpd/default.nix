...
let
  ...
  php = mainCfg.phpPackage.override {
    apacheHttpd = httpd.dev; # otherwise it only gets .out
  };

  phpMajorVersion = head (splitString "." php.version);

  mod_perl =
    pkgs.apacheHttpdPackages.mod_perl.override { apacheHttpd = httpd; };
  ...
  getListen = cfg:
    let
      list = (lib.optional (cfg.port != 0) {
        ip = "*";
        port = cfg.port;
      }) ++ cfg.listen;
    in if list == [ ] then defaultListen cfg else list;
  ...
  makeServerInfo = cfg: {
    ...
    # a virtual host.
    adminAddr =
      if cfg.adminAddr != null then cfg.adminAddr else mainCfg.adminAddr;
    ...
    let
      f = svc:
        let
          ...
          else if svc ? serviceExpression then
          ...
          res = defaults
            // svcFunction { inherit config lib pkgs serverInfo php; };
        in res;
    in map f defs;

  # !!! callSubservices is expensive
  subservicesFor = cfg:
    callSubservices (makeServerInfo cfg) cfg.extraSubservices;

  mainSubservices = subservicesFor mainCfg;

  allSubservices = mainSubservices
    ++ concatMap subservicesFor mainCfg.virtualHosts;

  enableSSL = any (vhost: vhost.enableSSL) allHosts;

  # Names of modules from ${httpd}/modules that we want to load.
  apacheModules = [ # HTTP authentication mechanisms: basic and digest.
    "auth_basic"
    "auth_digest"
    ...
    "access_compat"
  ] ++ (if mainCfg.multiProcessingModule == "prefork" then
    [ "cgi" ]
  else
    [ "cgid" ]) ++ optional enableSSL "ssl" ++ extraApacheModules;
  ...
  perServerConf = isMainServer: cfg:
    let
      ...
      maybeDocumentRoot = fold (svc: acc:
        if acc == null then
          ...
          assert svc.documentRoot == null; acc) null ([ cfg ] ++ subservices);

      documentRoot = if maybeDocumentRoot != null then
        maybeDocumentRoot
      else
    ...
    in ''
      ...
      ${if isMainServer || maybeDocumentRoot != null then
        documentRootConf
      else
        ""}
  ...
  confFile = pkgs.writeText "httpd.conf" ''
    ...
    ${let
      ...
      '';
      allModules = concatMap (svc: svc.extraModulesPre) allSubservices ++ map
        (name: {
          inherit name;
          path = "${httpd}/modules/mod_${name}.so";
        }) apacheModules ++ optional mainCfg.enableMellon {
          name = "auth_mellon";
          path =
            "${pkgs.apacheHttpdPackages.mod_auth_mellon}/modules/mod_auth_mellon.so";
        } ++ optional enablePHP {
          name = "php${phpMajorVersion}";
          path = "${php}/modules/libphp${phpMajorVersion}.so";
        } ++ optional enablePerl {
          name = "perl";
    ...
    ${let
      makeVirtualHost = vhost: ''
        <VirtualHost ${
          concatStringsSep " " (map listenToString (getListen vhost))
        }>
...
in {
  ...
  config = mkIf config.services.httpd.enable {
    ...
    assertions = [{
      assertion = mainCfg.enableSSL == true -> mainCfg.sslServerCert != null
        && mainCfg.sslServerKey != null;
      message =
        "SSL is enabled for httpd, but sslServerCert and/or sslServerKey haven't been specified.";
    }];

    warnings = map (cfg:
      ''
        apache-httpd's port option is deprecated. Use listen = [{/*ip = "*"; */ port = ${
          toString cfg.port
        };}]; instead'') (lib.filter (cfg: cfg.port != 0) allHosts);
    ...
    environment.systemPackages = [ httpd ]
      ++ concatMap (svc: svc.extraPath) allSubservices;
    ...
    systemd.services.httpd = {
      ...
      path = [ httpd pkgs.coreutils pkgs.gnugrep ] ++ optional enablePHP
        pkgs.system-sendmail # Needed for PHP's mail() function.
        ++ concatMap (svc: svc.extraServerPath) allSubservices;

      environment = optionalAttrs enablePHP { PHPRC = phpIni; }
        // optionalAttrs mainCfg.enableMellon {
      ...
      serviceConfig.ExecReload =
        "${httpd}/bin/httpd -f ${httpdConf} -k graceful";
