...
let
  ...
  optint = name: value:
    optionalString (value != null && value != 0) "${name} ${toString value}";
  ...
  torRc = ''
    ...
    ${optionalString cfg.controlSocket.enable
    "ControlPort unix:${torRunDirectory}/control GroupWritable RelaxDirModeCheck"}
  ''
    # Client connection config
    + optionalString cfg.client.enable ''
      SOCKSPort ${cfg.client.socksListenAddress} ${
        toString cfg.client.socksIsolationOptions
      }
      SOCKSPort ${cfg.client.socksListenAddressFaster}
      ${opt "SocksPolicy" cfg.client.socksPolicy}

      ${optionalString cfg.client.transparentProxy.enable ''
        TransPort ${cfg.client.transparentProxy.listenAddress} ${
          toString cfg.client.transparentProxy.isolationOptions
        }
      ''}

      ${optionalString cfg.client.dns.enable ''
        DNSPort ${cfg.client.dns.listenAddress} ${
          toString cfg.client.dns.isolationOptions
        }
        AutomapHostsOnResolve 1
        AutomapHostsSuffixes ${
          concatStringsSep "," cfg.client.dns.automapHostsSuffixes
        }
      ''}
    ''
    ...
    + optionalString cfg.relay.enable ''
      ...
      ${optionalString (elem cfg.relay.role [ "bridge" "private-bridge" ]) ''
        ...
        ServerTransportPlugin ${
          concatStringsSep "," cfg.relay.bridgeTransports
        } exec ${pkgs.obfs4}/bin/obfs4proxy managed
        ExtORPort auto
    ...
    + concatStrings (flip mapAttrsToList cfg.hiddenServices (n: v: ''
      ...
      ${optionalString (v.version != null)
      "HiddenServiceVersion ${toString v.version}"}
      ${flip concatMapStrings v.map (p: ''
        HiddenServicePort ${toString p.port} ${p.destination}
      '')}
      ${optionalString (v.authorizeClient != null) ''
        HiddenServiceAuthorizeClient ${v.authorizeClient.authType} ${
          concatStringsSep "," v.authorizeClient.clientNames
        }
      ''}
    '')) + cfg.extraConfig;
    ...
    services.tor = {
      ...
      client = {
        ...
        socksIsolationOptions =
          mkOption (isolationOptions // { default = [ "IsolateDestAddr" ]; });
        ...
        type = types.loaOf (types.submodule ({ name, ... }: {
          options = {
            ...
            map = mkOption {
              ...
              type = types.listOf (types.submodule ({ config, ... }: {
                ...
                config = {
                  ...
                  destination =
                    mkDefault "${config.toHost}:${toString config.toPort}";
                };
              }));
            };
            ...
            version = mkOption {
              default = null;
              description =
                "Rendezvous service descriptor version to publish for the hidden service. Currently, versions 2 and 3 are supported. (Default: 2)";
