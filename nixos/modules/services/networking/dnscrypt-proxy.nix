...
let
  ...
  upstreamResolverListPubKey = pkgs.fetchurl {
    url =
      "https://raw.githubusercontent.com/dyne/dnscrypt-proxy/master/minisign.pub";
  ...
  resolverArgs = if (cfg.customResolver == null) then [
    "-L ${resolverList}"
    "-R ${cfg.resolverName}"
  ] else
    with cfg.customResolver; [
      "-N ${name}"
      "-k ${key}"
      "-r ${address}:${toString port}"
    ];

  daemonArgs = [ "-a ${localAddress}" ] ++ resolverArgs ++ cfg.extraArgs;
...
in {
  ...
  config = mkIf cfg.enable (mkMerge [
    {
      ...
      systemd.services.dnscrypt-proxy = {
        ...
        serviceConfig = {
          NonBlocking = "true";
          ExecStart =
            "${pkgs.dnscrypt-proxy}/bin/dnscrypt-proxy ${toString daemonArgs}";
    ...
    (mkIf config.security.apparmor.enable {
      ...
      security.apparmor.profiles = singleton
  ...
  imports = [
    (mkRenamedOptionModule [ "services" "dnscrypt-proxy" "port" ] [
      "services"
      "dnscrypt-proxy"
      "localPort"
    ])

    (mkChangedOptionModule [ "services" "dnscrypt-proxy" "tcpOnly" ] [
      "services"
      "dnscrypt-proxy"
      "extraArgs"
    ] (config:
