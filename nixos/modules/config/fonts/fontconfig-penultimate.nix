...
let
  ...
  supportFontsConf = pkgs.makeFontsConf {
    fontconfig = supportPkg;
  ...
  makeCacheConf = { version ? null }:
    let
      fcPackage =
        if version == null then "fontconfig" else "fontconfig_${version}";
  ...
  penultimateConf =
    pkgs.runCommand "font-penultimate-conf" { preferLocalBuild = true; } ''
...
in {
  ...
  config = mkIf (config.fonts.fontconfig.enable
    && config.fonts.fontconfig.penultimate.enable) {
