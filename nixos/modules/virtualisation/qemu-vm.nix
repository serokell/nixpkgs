...
let
  ...
  vmName = if config.networking.hostName == "" then
    "noname"
  else
  ...
  mkDiskIfaceDriveFlag = idx: driveArgs:
    let
      ...
      args =
        "-drive ${commonArgs},if=none -device lsi53c895a -device ${devArgs}";
  ...
  startVM = ''
    ...
    ${flip concatMapStrings cfg.emptyDiskImages (size: ''
      ...
      extraDisks="$extraDisks ${
        mkDiskIfaceDriveFlag "$idx" "file=$(pwd)/empty$idx.qcow2,werror=report"
      }"
      idx=$((idx + 1))
    '')}
  ...
  bootDisk = pkgs.vmTools.runInLinuxVM (pkgs.runCommand "nixos-boot-disk" {
    ...
    QEMU_OPTS = if cfg.useEFIBoot then
      "-pflash $out/bios.bin -nographic -serial pty"
    else
...
in {
  ...
  config = {
    ...
    virtualisation.bootDevice = mkDefault
      (if cfg.qemu.diskInterface == "scsi" then "/dev/sda" else "/dev/vda");
    ...
    services.xserver.resolutions = mkVMOverride [{
      x = 1024;
      y = 768;
    }];
