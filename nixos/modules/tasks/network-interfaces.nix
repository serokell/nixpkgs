  ...
  interfaceOpts = { name, ... }: {
    ...
    options = {
      ...
      virtualType = mkOption {
        ...
        defaultText =
          literalExample ''if hasPrefix "tun" name then "tun" else "tap"'';
...
in {
        ...
        attrsOf (submodule {
          ...
          options = {
            ...
            device = mkOption {
              ...
              example = "wlp6s0";
              description =
                "The name of the underlying hardware WLAN device as assigned by <literal>udev</literal>.";
            };
            ...
            meshID = mkOption {
              ...
              default = null;
              description =
                "MeshID of interface with type <literal>mesh</literal>.";
            };
            ...
            fourAddr = mkOption {
              ...
              default = null;
              description =
                "Whether to enable <literal>4-address mode</literal> with type <literal>managed</literal>.";
            };
  ...
  config = {
    ...
    assertions = (flip map interfaces (i: {
    ...
    } // listToAttrs (flip concatMap (filter (i: i.proxyARP) interfaces) (i:
      flip map [ "4" "6" ]
    ...
    system.activationScripts.hostname = optionalString (cfg.hostName != "") ''
    ...
    environment.systemPackages =
      [ pkgs.host pkgs.iproute pkgs.iputils pkgs.nettools ]
    ...
    systemd.services = {
      ...
      let
        deviceDependency = if (config.boot.isContainer || i.name == "lo") then
          [ ]
        else
          [ (subsystemDevice i.name) ];
      in nameValuePair "network-link-${i.name}" {
      ...
      text = let
        ...
        curInterfaceScript = device: current: new:
          pkgs.writeScript "udev-run-script-wlan-interfaces-${device}.sh" ''
            ...
            "${pkgs.iproute}/bin/ip link set dev ${device} address ${current.mac}"}
          '';

        # Udev script to execute for a new WLAN interface. The script configures the new WLAN interface.
        newInterfaceScript = device: new:
