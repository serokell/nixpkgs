...
let
  ...
  dhcpStr = useDHCP:
    if useDHCP == true || useDHCP == null then "both" else "no";
...
in {
  ...
  config = mkIf cfg.useNetworkd {
    ...
    assertions = [
      ...
      {
        assertion = cfg.defaultGateway == null || cfg.defaultGateway.interface
          == null;
        message =
          "networking.defaultGateway.interface is not supported by networkd.";
      }
      {
        assertion = cfg.defaultGateway6 == null || cfg.defaultGateway6.interface
          == null;
        message =
    ...
    in mkMerge [
      ...
      (mkMerge (flip mapAttrsToList cfg.bonds (name: bond: {
        netdevs."40-${name}" = {
          ...
          bondConfig = let
            ...
            buildOptionSet = mapAttrs (_: kOpts:
              with kOpts;
      ...
      (mkMerge (flip mapAttrsToList cfg.vlans (name: vlan: {
        ...
        networks."40-${vlan.interface}" =
          (mkMerge [ (genericNetwork (mkOverride 999)) { vlan = [ name ]; } ]);
      })))
    ];
